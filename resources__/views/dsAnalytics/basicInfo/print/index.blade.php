...
@if (Request::get('view') == 'pdf' || Request::get('view') == 'print')
        ...
        <div class="portlet-body">
            <div class="row text-center">
                <div class="col-md-12 text-center">
                    <img width="500" height="auto" src="{{ $basePath }}/public/img/sint_ams_logo.png"
                        alt="" />
                </div>
            </div>
            <div class="row margin-top-10">
                <div class="col-md-12">
                    <div class="text-center bold uppercase">
                        <span class="header">@lang('label.BASIC_INFORMATION_WISE_DS_ANALYTICS')</span>
                    </div>
                </div>
            </div>
            <div class="row margin-top-10">
                <div class="col-md-12">
                    <div class="bg-blue-hoki bg-font-blue-hoki">
                        <h5 style="padding: 10px;">
                            {{ __('label.NAME') }} :
                            <strong>{{ !empty(Request::get('name')) && Request::get('name') != '' ? Request::get('name') : __('label.N_A') }}
                                |</strong>
                            {{ __('label.SERVICE') }} :
                            <strong>{{ !empty($wingList[Request::get('wing_id')]) && Request::get('wing_id') != 0 ? $wingList[Request::get('wing_id')] : __('label.ALL') }}
                                |</strong>
                            {{ __('label.RANK') }} :
                            <strong>{{ !empty($rankList[Request::get('rank')]) && Request::get('rank_id') != 0 ? $rankList[Request::get('rank')] : __('label.ALL') }}
                                |</strong>
                            {{ __('label.ARMS_SERVICE') }} :
                            <strong>{{ !empty($armsServiceList[Request::get('arms_service')]) && Request::get('arms_service') != 0 ? $armsServiceList[Request::get('arms_service')] : __('label.ALL') }}
                                |</strong>
                            {{ __('label.APPOINTMENT_NAME') }} :
                            <strong>{{ !empty(Request::get('appt_id')) && Request::get('appt_id') != 0 ? $appointmentList[Request::get('appt_id')] : __('label.N_A') }}
                                |</strong>
                            {{ __('label.BLOOD_GROUP') }} :
                            <strong>{{ !empty($bloodGroupList[Request::get('blood_group')]) && Request::get('blood_group') != 0 ? $bloodGroupList[Request::get('blood_group')] : __('label.ALL') }}
                                |</strong>
                            {{ __('label.BIRTH_DATE_FROM') }} :
                            <strong>{{ !empty(Request::get('birth_date_from')) && Request::get('birth_date_from') != '' ? Request::get('birth_date_from') : __('label.N_A') }}
                                |</strong>
                            {{ __('label.BIRTH_DATE_TO') }} :
                            <strong>{{ !empty(Request::get('birth_date_to')) && Request::get('birth_date_to') != '' ? Request::get('birth_date_to') : __('label.N_A') }}
                                |</strong>
                            {{ __('label.RELIGION') }} :
                            <strong>{{ !empty($religionList[Request::get('religion')]) && Request::get('religion') != 0 ? $religionList[Request::get('religion')] : __('label.ALL') }}
                                |</strong>
                            {{ __('label.GENDER') }} :
                            <strong>{{ !empty($genderList[Request::get('gender')]) && Request::get('gender') != 0 ? $genderList[Request::get('gender')] : __('label.ALL') }}
                                |</strong>
                            {{ __('label.TOTAL_NO_OF_DS') }} :
                            <strong>{{ !empty($targetArr) ? sizeof($targetArr) : 0 }}</strong>
                    ...
                    <table class="table table-bordered table-hover table-head-fixer-color" id="dataTable">
                        <thead>
                            <tr>
                                ...
                                @if (empty($columns) || in_array('9', $columns))
                                ...
                                @endif
                                @if (empty($columns) || in_array('8', $columns))
                            ...
                            @if (!empty($targetArr))
                                ...
                                @foreach ($targetArr as $id => $target)
                                    <tr>
                                        ...
                                        @if (empty($columns) || in_array('1', $columns))
                                            <td class="vcenter text-center" width="50px">
                                                @if (!empty($target['photo']) && File::exists('public/uploads/user/' . $target['photo']))
                                                    <img class="profile-zoom" width="50" height="60"
                                        ...
                                        @endif
                                        @if (empty($columns) || in_array('5', $columns))
                                            <td class="vcenter text-center">{!! !empty($target['blood_group']) && !empty($bloodGroupList[$target['blood_group']])
                                                ? $bloodGroupList[$target['blood_group']]
                                                : '' !!}</td>
                                        @endif
                                        @if (empty($columns) || in_array('6', $columns))
                                        ...
                                        @endif
                                        @if (empty($columns) || in_array('7', $columns))
                                            <td class="vcenter text-center">{!! !empty($target['religion_id']) && !empty($religionList[$target['religion_id']])
                                                ? $religionList[$target['religion_id']]
                                                : '' !!}</td>
                                        @endif
                                        @if (empty($columns) || in_array('8', $columns))
    ...
    <div class="width-inherit">
        <a class="btn btn-xs green-seagreen tooltips vcenter" title="@lang('label.CLICK_HERE_TO_VIEW_PROFILE')" href="{!! URL::to('basicInformationDSAnalytics/' . $target['id'] . '/profile' . Helper::queryPageStr($qpArr)) !!}">
...
@else
    ...
    <head>
        <link href="{{ asset('public/fonts/css.css?family=Open Sans') }}" rel="stylesheet" type="text/css">
        <link href="{{ asset('public/assets/global/plugins/font-awesome/css/font-awesome.min.css') }}"
        ...
        <table class="no-border margin-top-10">
            ...
            <tr>
                <td class="no-border text-center" colspan="8">
                    <strong>{!! __('label.BASIC_INFORMATION_WISE_DS_ANALYTICS') !!}</strong>
                </td>
            </tr>
        </table>
        <table class="no-border margin-top-10">
