...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-line-chart"></i>@lang('label.CM_GROUP_WISE_EVENT_TREND')
            </div>
        </div>
        <div class="portlet-body">
            {!! Form::open(array('group' => 'form', 'url' => 'cmGroupWiseEventTrendReportCrnt/filter','class' => 'form-horizontal', 'id' => 'submitForm')) !!}
            <div class="row">
                <div class="col-md-2">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <div class="col-md-8" id ="showCmGroup">
                            {!! Form::select('cm_group_id[]', $cmGroupList, $cmGroupIds,  ['multiple'=>'multiple', 'class' => 'form-control', 'id' => 'cmGroupId', 'data-width' => '100%']) !!}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div id="cmGroupWiseEventTrendChart" style="width: 100%; height: 400px; margin: 0 auto;"></div>
                </div>
            </div>
...
var cmGroupAllSelected = false;
$('#cmGroupId').multiselect({
numberDisplayed: 0,
        includeSelectAllOption: true,
        buttonWidth: 'inherit',
        maxHeight: 250,
        nonSelectedText: "@lang('label.SELECT_CM_GROUP_OPT')",
        enableCaseInsensitiveFiltering: true,
        onSelectAll: function () {
        cmGroupAllSelected = true;
        },
        onChange: function () {
        cmGroupAllSelected = false;
        }
});
...
$.ajax({
url: "{{ URL::to('cmGroupWiseEventTrendReportCrnt/getCourse')}}",
        type: "POST",
        dataType: "json",
        headers: {
...
$.ajax({
url: "{{ URL::to('cmGroupWiseEventTrendReportCrnt/getTerm')}}",
        type: "POST",
        dataType: "json",
        headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
$.ajax({
url: "{{ URL::to('cmGroupWiseEventTrendReportCrnt/getCourseWiseCmGroupEvent')}}",
        type: "POST",
        dataType: "json",
        headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
<?php
if (!empty($selectedCmGroups)) {
    foreach ($selectedCmGroups as $cmGroupId => $cmGroupName) {
        ?>
        counter2 = 0;
        maxMinArr[counter1] = [];
        <?php
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $max = !empty($cmGroupWiseMksArr[$cmGroupId][$eventId]['max']) ? $cmGroupWiseMksArr[$cmGroupId][$eventId]['max'] : 0;
                $min = !empty($cmGroupWiseMksArr[$cmGroupId][$eventId]['min']) ? $cmGroupWiseMksArr[$cmGroupId][$eventId]['min'] : 0;
                ?>
                max = <?php echo $max; ?>;
                min = <?php echo $min; ?>;
...
if (!empty($selectedCmGroups)) {
    foreach ($selectedCmGroups as $cmGroupId => $cmGroupName) {
        if (sizeof($selectedCmGroups) > 2) {
            ...
            if (!empty($selectedEvents)) {
                foreach ($selectedEvents as $eventId => $eventCode) {
                    $mks = !empty($cmGroupWiseMksArr[$cmGroupId][$eventId]['mks_percent']) ? $cmGroupWiseMksArr[$cmGroupId][$eventId]['mks_percent'] : 0;
                    echo $mks . ',';
                }
            }
            ?>
            ...
            ?>
                    {
                    name: "{{$cmGroupName}} (@lang('label.MAX'))",
                            data: [
            <?php
            if (!empty($selectedEvents)) {
                foreach ($selectedEvents as $eventId => $eventCode) {
            ...
            ?>
                    ...
                    {
                    name: "{{$cmGroupName}} (@lang('label.AVG'))",
                            data: [
            <?php
            if (!empty($selectedEvents)) {
                foreach ($selectedEvents as $eventId => $eventCode) {
            ...
            ?>
                    ...
                    {
                    name: "{{$cmGroupName}} (@lang('label.MIN'))",
                            data: [
            <?php
            if (!empty($selectedEvents)) {
                foreach ($selectedEvents as $eventId => $eventCode) {
...
if (!empty($selectedCmGroups)) {
    foreach ($selectedCmGroups as $cmGroupId => $cmGroupName) {
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $mks = !empty($cmGroupWiseMksArr[$cmGroupId][$eventId]['max']) ? $cmGroupWiseMksArr[$cmGroupId][$eventId]['max'] : 0;
                echo $mks . ',';
            }
        }
    }
...
if (!empty($selectedCmGroups)) {
    foreach ($selectedCmGroups as $cmGroupId => $cmGroupName) {
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $mks = !empty($cmGroupWiseMksArr[$cmGroupId][$eventId]['min']) ? $cmGroupWiseMksArr[$cmGroupId][$eventId]['min'] : 0;
                echo $mks . ',';
            }
        }
    }
...
if (!empty($selectedEvents)) {
    if (sizeof($selectedEvents) > 1) {
        ?>
        var cmGroupWiseMks = new ApexCharts(document.querySelector("#cmGroupWiseEventTrendChart"), cmGroupWiseMksOptions);
        <?php
    } else {
        ?>
        var cmGroupWiseMks = new ApexCharts(document.querySelector("#cmGroupWiseEventTrendChart"), cmGroupWiseMksOptions2);
        <?php
    }
    ?>
    cmGroupWiseMks.render();
    <?php
}
?>
