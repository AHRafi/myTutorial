                    ...
                    <table class="table table-bordered table-hover relation-view-2">
                        <thead>
                            <tr class="active">
                                ...
                                <th class="text-center vcenter">@lang('label.HAS_SUB_SUB_SUB_EVENT')</th>
                                <th class="text-center vcenter {{ (($prevDsAssesment->has_ds_assesment) == '1') ? 'display-none' : '' }}">
                        ...
                        <tbody id="exerciseData">
                            @if(!$assignedSubSubEventArr->isEmpty())
                            @php $sl = 0 @endphp
                            @foreach($assignedSubSubEventArr as $assignedSubSubEvent)
                            <tr>
                                <td class="text-center vcenter">{!! ++$sl !!}</td>
                                <td class="vcenter">{{ $assignedSubSubEvent->event_code }}</td>
                                <td class="text-center vcenter">
                                    @if($assignedSubSubEvent->has_sub_sub_sub_event == '1')
                                    <span class="label label-sm label-success">@lang('label.YES')</span>
                                    @else
                                    <span class="label label-sm label-warning">@lang('label.NO')</span>
                                    @endif
                                </td>
                                <td class="text-center vcenter {{ (($prevDsAssesment->has_ds_assesment) == '1') ? 'display-none' : '' }}">
                                    @if($assignedSubSubEvent->has_ds_assesment == '1')
                                    <span class="label label-sm label-success">@lang('label.YES')</span>
                                    @else
                                    <span class="label label-sm label-warning">@lang('label.NO')</span>
                                    @endif
                            ...
                            <tr>
                                <td colspan="4" class="text-danger">
                                    @lang('label.NO_SUB_SUB_EVENT_FOUND_FOR_THIS_EVENT')
                                </td>
                            </tr>
                            @endif      
                        </tbody>
