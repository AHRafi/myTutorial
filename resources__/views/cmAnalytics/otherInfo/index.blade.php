...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-line-chart"></i>@lang('label.OTHER_INFORMATION_WISE_CM_ANALYTICS')
            </div>
        </div>
        <div class="portlet-body">
            <!-- Begin Filter-->
            {!! Form::open(array('group' => 'form', 'url' => 'otherInfoWiseCmAnalytics/filter','class' => 'form-horizontal')) !!}
            <div class="row">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label class="control-label col-md-4" for="name">@lang('label.DECORATION_N_AWARD')</label>
                        <div class="col-md-8">
                            {!! Form::text('award',  Request::get('award'), ['class' => 'form-control', 'id' => 'name',  'placeholder' => 'Award', 'autocomplete' => 'off']) !!} 
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label class="control-label col-md-4" for="name">@lang('label.EXTRA_CURRICULAR_EXP')</label>
                        <div class="col-md-8">
                            {!! Form::text('extra_cur',  Request::get('extra_cur'), ['class' => 'form-control tooltips', 'id' => 'name', 'placeholder' => 'Extra Curricular Expertise', 'autocomplete' => 'off']) !!} 
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    <div class="bg-blue-hoki bg-font-blue-hoki">
                        <h5 style="padding: 10px;">
                            ...
                            {{__('label.DECORATION_N_AWARD')}} : <strong>{{ !empty(Request::get('award')) && Request::get('award') != '' ? Request::get('award') : __('label.N_A') }} |</strong>
                            {{__('label.EXTRA_CURRICULAR_EXP')}} : <strong>{{ !empty(Request::get('extra_cur')) && Request::get('extra_cur') != '' ? Request::get('extra_cur') : __('label.N_A') }} |</strong>
                        ...
                        <table class="table table-bordered table-hover table-head-fixer-color" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="vcenter">@lang('label.DECORATION_AWARD')</th>
                                    <th class="vcenter">@lang('label.EXTRA_CURRICULAR_EXPERTISE')</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td width="200">
                                        @if(!empty($target['decoration_id']))
                                        <?php
                                        $slNo = 0;
                                        $decArr = !empty($target['decoration_id']) ? explode(',', $target['decoration_id']) : [];
                                        ?>
                                        @if(!empty($decArr))


                                        @foreach($decArr as $dec)
                                        {!! ++$slNo !!}.&nbsp;{!! !empty($dec) ? $dec : '' !!}{!! '<br />' !!}
                                        @endforeach
                                    ...
                                    <td width="200">
                                        @if(!empty($target['extra_curriclar_expt']))
                                        <?php
                                        $slNo = 0;
                                        $exptArr = !empty($target['extra_curriclar_expt']) ? explode(',', $target['extra_curriclar_expt']) : [];
                                        ?>
                                        @if(!empty($exptArr))


                                        @foreach($exptArr as $expt)
                                        {!! ++$slNo !!}.&nbsp;{!! !empty($expt) ? $expt : '' !!}{!! '<br />' !!}
                                        @endforeach
