                ...
                <table class="table table-bordered table-hover table-head-fixer-color">
                    <thead>
                        <tr>
                            ...
                            <th class="vcenter text-center">@lang('label.CM_FORCE_SUBMIT')</th>
                            @endif
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center vcenter">
                                ...
                                @if(!empty($forceSubmitLockInfo[$dsInfo['ds_id']]) && $forceSubmitLockInfo[$dsInfo['ds_id']] == '1')
                                <span class="label label-sm label-yellow-casablanca">@lang('label.FORCE_SUBMITTED_BY_DS_COORD')</span>
                                @else
                            ...
                            @if($request->data_id == '2')
							<td class="text-center vcenter">
                                <a type="button" class = "btn bold btn-warning tooltips force-sumbit-cm" ds-id ="{{ $dsInfo['ds_id'] }}"
                                    course-id='{{$request->course_id }}'  term-id='{{$request->term_id }}' 
                                    event-id='{{$request->event_id}}' sub-event-id='{{$request->sub_event_id}}' sub-sub-event-id='{{$request->sub_sub_event_id}}' 
                                    sub-sub-sub-event-id='{{$request->sub_sub_sub_event_id}}'
                                    title="@lang('label.CLICK_HERE_TO_FORCE_SUBMIT_CM')">
                                     {{__('label.FORCE_SUBMIT')}}
                                 </a>
                            </td>
                            @endif
                        </tr>
