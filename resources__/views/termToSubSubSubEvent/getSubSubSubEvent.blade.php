...
<!--    @if(!empty($eventAssessmentMarkingData))
    ...
    <div class="col-md-9">
        ...
        <span class="label label-sm label-green-seagreen">
            @lang('label.TOTAL_NO_OF_ASSIGNED_SUB_SUB_SUB_EVENTS'):&nbsp;{!! !empty($prevDataList) ? sizeOf($prevDataList) : 0 !!}
        </span>&nbsp;
        <button class="label label-sm label-purple btn-label-groove tooltips" href="#modalAssignedSubSubSubEvent" id="assignedSubSubSubEvent"  data-toggle="modal" title="@lang('label.SHOW_ASSIGNED_SUB_SUB_SUB_EVENT')">
            @lang('label.SUB_SUB_SUB_EVENT_ASSIGNED_TO_THIS_TERM'):&nbsp;{!! !empty($prevTermToSubSubSubEventList) ? sizeOf($prevTermToSubSubSubEventList) : 0 !!}&nbsp; <i class="fa fa-search-plus"></i>
        </button>
    </div>
    @if(empty($eventAssessmentMarkingData))
...
//                    }
                ...
                if (!empty($prevSaveTermToSubSubSubEventList[$target->id]) && ($request->term_id != $prevSaveTermToSubSubSubEventList[$target->id])) {
                    $disabled = 'disabled';
                    $term = !empty($prevSaveTermToSubSubSubEventList[$target->id]) && !empty($termList[$prevSaveTermToSubSubSubEventList[$target->id]]) ? $termList[$prevSaveTermToSubSubSubEventList[$target->id]] : '';
                    $title = __('label.ALREADY_ASSIGNED_TO_THIS_TERM', ['term' => $term]);
                }
                if (!empty($eventAssessmentMarkingData)) {
                ...
                <tr>
                    ...
                    <td class="vcenter">
                        <div class="md-checkbox has-success tooltips" title="{!!$title!!}" >
                            {!! Form::checkbox('sub_sub_sub_event_id['.$target->id.']',$target->id, $checked, ['id' => $target->id, 'data-id'=>$target->id,'class'=> 'md-check '.$class,$disabled]) !!}
                            @if (($disabled == 'disabled') && ($checked == 'checked'))
                            {!! Form::hidden('sub_sub_sub_event_id['.$target->id.']', $target->id) !!}
                            @endif
