...
<!--    @if(!empty($eventAssessmentMarkingData))
    ...
    <div class="col-md-8 margin-top-10">
        <span class="label label-md bold label-blue-steel">
            @lang('label.TOTAL_SUB_SUB_SUB_EVENT'):&nbsp;{!! sizeof($subSubSubEventArr) !!}
        </span>&nbsp;
        <span class="label label-md bold label-green-soft">
            @lang('label.TOTAL_SUB_SUB_EVENT_WT'):&nbsp;{!! $assignedMksWtArr->wt ?? '0.00' !!}
            {!! Form::hidden('total_event_wt', !empty($assignedMksWtArr->wt) ? $assignedMksWtArr->wt : null,['id' => 'totalEventWt'] )!!}
            {!! Form::hidden('avg_marking', !empty($avgMarkingArr) ? 1 : 0,['id' => 'avgMarking'] )!!}
            {!! Form::hidden('total_event', !empty($avgMarkingArr) ? sizeof($subSubSubEventArr) : 1,['id' => 'totalEvent'] )!!}
        </span>
    </div>
    @if(empty($eventAssessmentMarkingData))
    ...
    <div class="col-md-12 margin-top-10">
        <table class="table table-bordered table-hover">
            ...
            <tbody>
                ...
                $i = 0;
                ?>
                @foreach($subSubSubEventArr as $subSubSubEventId => $subSubSubEventName)
                <?php
                $readOnly = '';
                if (!empty($eventAssessmentMarkingData)) {
                    if (array_key_exists($subSubSubEventId, $eventAssessmentMarkingData)) {
                        $readOnly = !empty($eventAssessmentMarkingData) ? 'readonly' : '';
                    }
                }
                $eventMksLimit = !empty($subSubSubEventMksWtArr[$subSubSubEventId]['mks_limit']) ? $subSubSubEventMksWtArr[$subSubSubEventId]['mks_limit'] : (!empty($subSubSubEventMksWtArr['mks_limit']) ? $subSubSubEventMksWtArr['mks_limit'] : null);
                $eventHighestMksLimit = !empty($subSubSubEventMksWtArr[$subSubSubEventId]['highest_mks_limit']) ? $subSubSubEventMksWtArr[$subSubSubEventId]['highest_mks_limit'] : (!empty($subSubSubEventMksWtArr['highest_mks_limit']) ? $subSubSubEventMksWtArr['highest_mks_limit'] : null);
                $eventLowestMksLimit = !empty($subSubSubEventMksWtArr[$subSubSubEventId]['lowest_mks_limit']) ? $subSubSubEventMksWtArr[$subSubSubEventId]['lowest_mks_limit'] : (!empty($subSubSubEventMksWtArr['lowest_mks_limit']) ? $subSubSubEventMksWtArr['lowest_mks_limit'] : null);
                $eventWt = !empty($subSubSubEventMksWtArr[$subSubSubEventId]['wt']) ? $subSubSubEventMksWtArr[$subSubSubEventId]['wt'] : null;
                ?>
                <tr>
                    <td class="vcenter text-center">{!! ++$i !!}</td>
                    <td class="vcenter">{!! $subSubSubEventName ?? '' !!}</td>
                    <td class="vcenter width-80">
                        {!! Form::text('event_mks_wt['.$subSubSubEventId.'][mks]', $eventMksLimit,['id' => 'mksLimit_'.$subSubSubEventId, 'data-key' => $subSubSubEventId, 'class' => 'mks-limit form-control integer-decimal-only width-inherit text-right','autocomplete' => 'off', $readOnly]) !!}
                    </td>
                    <td class="vcenter width-80">
                        {!! Form::text('event_mks_wt['.$subSubSubEventId.'][highest]', $eventHighestMksLimit,['id' => 'highestLimit_'.$subSubSubEventId, 'data-key' => $subSubSubEventId, 'class' => 'highest-mks form-control integer-decimal-only width-inherit text-right','autocomplete' => 'off', $readOnly]) !!}
                    </td>
                    <td class="vcenter width-80">
                        {!! Form::text('event_mks_wt['.$subSubSubEventId.'][lowest]', $eventLowestMksLimit,['id' => 'lowestLimit_'.$subSubSubEventId, 'data-key' => $subSubSubEventId, 'class' => 'lowest-limit form-control integer-decimal-only width-inherit text-right','autocomplete' => 'off', $readOnly]) !!}
                    </td>
                    <td class="vcenter width-80">
                        {!! Form::text('event_mks_wt['.$subSubSubEventId.'][wt]', $eventWt,['id' => 'wt_'.$subSubSubEventId, 'data-key' => $subSubSubEventId, 'class' => 'wt-distributed form-control integer-decimal-only width-inherit text-right','autocomplete' => 'off', $readOnly]) !!}
                    </td>
                </tr>
                @endforeach
    ...
    <div class="col-md-12 text-center">
        ...
        <a href="{{ URL::to('subSubSubEventMksWt') }}" class="btn btn-circle btn-outline grey-salsa">@lang('label.CANCEL')</a>
    </div>
    @else
    <div class="col-md-12">
        <div class="alert alert-danger alert-dismissable">
            <p><strong><i class="fa fa-bell-o fa-fw"></i> {!! __('label.NO_SUB_SUB_SUB_EVENT_IS_ASSIGNED_TO_THIS_SUB_SUB_EVENT') !!}</strong></p>
        </div>
    </div>
    @endif
    @else
    <div class="col-md-12">
        <div class="alert alert-danger alert-dismissable">
            <p><strong><i class="fa fa-bell-o fa-fw"></i> {!! __('label.SUB_SUB_EVENT_WT_IS_NOT_ASSIGNED_YET') !!}</strong></p>
        </div>
    </div>
    @endif
</div>
...
$(document).ready(function () {
    ...
    function total() {
        ...
        if (avgMarking == 1) {
            sum = parseFloat(sum) / parseFloat(totalEvent);
            sum = parseFloat(sum);
        }
        $("#totalWt").val(sum);
