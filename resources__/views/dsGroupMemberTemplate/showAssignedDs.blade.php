<div class="modal-content">
    ...
    <div class="modal-body">
        <div class="row margin-bottom-10">
            ...
            <div class="col-md-4">
                @lang('label.DS_GROUP'): <strong>{!! $dsGroupName->name ?? '' !!}</strong>
            </div>
        </div>
                        ...
                        <tbody id="exerciseData">
                            ...
                            <tr>
                                ...
                                <td class="text-center vcenter" width="50px">
                                    <?php if (!empty($assignedDs->photo && File::exists('public/uploads/user/' . $assignedDs->photo))) { ?>
                                        <img width="50" height="60" src="{{URL::to('/')}}/public/uploads/user/{{$assignedDs->photo}}" alt="{{ $assignedDs->official_name}}"/>
                                    <?php } else { ?>
                                        <img width="50" height="60" src="{{URL::to('/')}}/public/img/unknown.png" alt="{{ $assignedDs->official_name}}"/>
                                    <?php } ?>
                                </td>
                                <td class="vcenter">{{ $assignedDs->personal_no }}</td>
                                <td class="vcenter">{{ $assignedDs->rank_code }}</td>
                                <td class="vcenter">{!! $assignedDs->official_name !!}</td>
                            ...
                            <tr>
                                <td colspan="6" class="text-danger">
                                    @lang('label.NO_DS_FOUND_FOR_THIS_DS_GROUP')
                                </td>
                            </tr>
                            @endif      
                        </tbody>
