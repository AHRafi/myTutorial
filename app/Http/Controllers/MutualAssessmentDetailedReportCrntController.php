...
class MutualAssessmentDetailedReportCrntController extends Controller {

    private $controller = 'MutualAssessmentDetailedReportCrnt';
    ...
    public function index(Request $request) {
        ...
        if (empty($activeTrainingYearList)) {
            $void['header'] = __('label.MUTUAL_ASSESSMENT_DETAILED_REPORT');
...
//            exit;
            ...
            $fileName = 'Mutual_Assessment_Detailed_Report' . $tyName . $courseName . $termName . $synName . $subSynName . $eventName . $subEventName . $subSubEventName . $subSubSubEventName . $eventGpName;
            $fileName = Common::getFileFormatedName($fileName);
        }

        if ($request->view == 'print') {
            return view('reportCrnt.mutualAssessmentDetailed.print.index')->with(compact('activeTrainingYearList', 'courseList', 'termList'
            ...
            $pdf = PDF::loadView('reportCrnt.mutualAssessmentDetailed.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'factorList', 'synList', 'subSynList', 'hasSubSyn', 'cmArr', 'markingPositionArr'
                                    , 'totalPositionArr', 'markingCmArr', 'sortByList', 'eventGroupList'
                                    , 'maProcess', 'eventList', 'subEventList', 'subSubEventList', 'subSubSubEventList'))
            ...
            return Excel::download(new ExcelExport('reportCrnt.mutualAssessmentDetailed.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'factorList', 'synList', 'subSynList', 'hasSubSyn', 'cmArr', 'markingPositionArr'
                                    , 'totalPositionArr', 'markingCmArr', 'sortByList', 'eventGroupList'
                                    , 'maProcess', 'eventList', 'subEventList', 'subSubEventList', 'subSubSubEventList')), $fileName . '.xlsx');
        }

        return view('reportCrnt.mutualAssessmentDetailed.index', compact('activeTrainingYearList', 'courseList', 'termList'
                        , 'factorList', 'synList', 'subSynList', 'hasSubSyn', 'cmArr', 'markingPositionArr'
                        , 'totalPositionArr', 'markingCmArr', 'sortByList', 'eventGroupList'
                        , 'maProcess', 'eventList', 'subEventList', 'subSubEventList', 'subSubSubEventList'));
    }
    ...
    public function getCourse(Request $request) {
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + Course::where('training_year_id', $request->training_year_id)
                        ...
                        ->pluck('name', 'id')
                        ->toArray();
        $html = view('reportCrnt.mutualAssessmentDetailed.getCourse', compact('courseList'))->render();
    ...
    public function getTerm(Request $request) {
        ...
        $html = view('reportCrnt.mutualAssessmentDetailed.getTerm', compact('termList'))->render();
        return Response::json(['html' => $html]);
    }
    ...
    public function getSubEvent(Request $request) {
        ...
        $html2 = view('reportCrnt.mutualAssessmentDetailed.showSubSubEvent', compact('subSubEventList'))->render();
        $html3 = view('reportCrnt.mutualAssessmentDetailed.showSubSubSubEvent', compact('subSubSubEventList'))->render();

        $html1 = Self::getEventGroups($request);
        $html = view('reportCrnt.mutualAssessmentDetailed.showSubEvent', compact('subEventList'))->render();
    ...
    public function getSubSubEvent(Request $request) {
        ...
        $subSubSubEventList = ['0' => __('label.SELECT_SUB_SUB_SUB_EVENT_OPT')];
        $html2 = view('reportCrnt.mutualAssessmentDetailed.showSubSubSubEvent', compact('subSubSubEventList'))->render();


        $html1 = Self::getEventGroups($request);
        $html = view('reportCrnt.mutualAssessmentDetailed.showSubSubEvent', compact('subSubEventList'))->render();
    ...
    public function getSubSubSubEvent(Request $request) {
        ...
        $html1 = Self::getEventGroups($request);
        $html = view('reportCrnt.mutualAssessmentDetailed.showSubSubSubEvent', compact('subSubSubEventList'))->render();
    ...
    public function getEventGroups(Request $request) {
        ...
        $html = view('reportCrnt.mutualAssessmentDetailed.showEventGroup', compact('eventGroupList'))->render();
    ...
    public function getSynOrGp(Request $request) {
        ...
        $html1 = view('reportCrnt.mutualAssessmentDetailed.showSubEvent', compact('subEventList'))->render();
        $html2 = view('reportCrnt.mutualAssessmentDetailed.showSubSubEvent', compact('subSubEventList'))->render();
        $html3 = view('reportCrnt.mutualAssessmentDetailed.showSubSubSubEvent', compact('subSubSubEventList'))->render();
        $html4 = view('reportCrnt.mutualAssessmentDetailed.showEventGroup', compact('eventGroupList'))->render();

        $html = view('reportCrnt.mutualAssessmentDetailed.getSyn', compact('synList', 'subSynList', 'eventList', 'maProcess'))->render();
    ...
    public function getsubSyn(Request $request) {
        ...
        if (sizeof($subSynList) > 1) {
            $html = view('reportCrnt.mutualAssessmentDetailed.getSubSyn', compact('subSynList'))->render();
        }
        return Response::json(['html' => $html]);
    }
...
//            $messages['event_group_id.not_in'] = __('label.EVENT_GROUP_MUST_BE_SELECTED');
        ...
        $url = 'training_year_id=' . $request->training_year_id . '&course_id=' . $request->course_id . '&term_id=' . $request->term_id
                . '&syn_id=' . $request->syn_id . '&sub_syn_id=' . $request->sub_syn_id
        ...
        if ($validator->fails()) {
            return redirect('mutualAssessmentDetailedReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('mutualAssessmentDetailedReportCrnt?generate=true&' . $url);
    }
