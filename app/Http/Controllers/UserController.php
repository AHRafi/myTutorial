...
class UserController extends Controller {
    ...
    public function index(Request $request) {
        $nameArr = User::select('official_name')->where('status', '1');
        if (Auth::user()->id != 125) {
            $nameArr = $nameArr->where('users.id', '<>', 125);
        }
...
//        ];
        ...
        $targetArr = User::with('rank', 'appointment')
                ...
                ->select('user_group.name as group_name', 'wing.code as wing_name'
        ...
        if (Auth::user()->id != 125) {
            $targetArr = $targetArr->where('users.id', '<>', 125);
        }
...
//        if (!empty($searchText)) {
        $targetArr->where(function ($query) use ($searchText) {
            $query->where('users.official_name', 'LIKE', '%' . $searchText . '%')
                    ->orWhere('users.personal_no', 'LIKE', '%' . $searchText . '%');
        });
//        }
...
//        }
    ...
    public function store(Request $request) {
        ...
        if (!empty($request->group_id) && in_array($request->group_id, ['3', '4'], TRUE)) {

//            $rules['father_name'] = 'required';
//            $rules['arms_service_id'] = 'required|not_in:0';
//            $rules['commissioning_course_id'] = 'required|not_in:0';
//            $rules['commission_type'] = 'required|not_in:0';
//            $rules['commisioning_date'] = 'required';
//            $rules['date_of_birth'] = 'required';
//            $rules['birth_place'] = 'required';
//            $rules['religion_id'] = 'required|not_in:0';
        }
...
//        exit;
        ...
        try {
            if ($target->save()) {
                if (in_array($request->group_id, [3, 4])) {
                    $newTarget = new UserBasicProfile;
    ...
    public function edit(Request $request, $id) {
        ...
        $target = User::leftJoin('user_basic_profile', 'user_basic_profile.user_id', '=', 'users.id')
                        ->select('users.id', 'users.group_id', 'users.wing_id', 'users.rank_id'
                                , 'users.appointment_id', 'users.personal_no', 'users.full_name'
                                , 'users.official_name', 'users.username', 'users.password', 'users.join_date'
                                , 'users.phone_official', 'users.email', 'users.phone', 'users.photo'
                                , 'users.status', 'user_basic_profile.father_name'
        ...
        return view('user.edit')->with(compact('target', 'qpArr', 'groupList', 'rankList', 'appointmentList', 'genderList'
                                , 'userNameArr', 'wingList', 'armsServiceList', 'religionList', 'commissioningCourseList', 'commissionTypeList', 'bloodGroupList'));
    }
    ...
    public function update(Request $request, $id) {
        ...
        $rules = [
            ...
            'rank_id' => 'required|not_in:0',
            'appointment_id' => 'required|not_in:0',
            'personal_no' => 'required|unique:users,personal_no,' . $id . ',id,group_id,' . $request->group_id,
            'full_name' => 'required',
            'official_name' => 'required',
            'username' => 'required|alpha_dash|unique:users,username,' . $id
        ];

        if (!empty($request->group_id) && in_array($request->group_id, ['3', '4'])) {
//            $rules['father_name'] = 'required';
            $rules['arms_service_id'] = 'required|not_in:0';
//            $rules['commissioning_course_id'] = 'required|not_in:0';
//            $rules['commission_type'] = 'required|not_in:0';
...
//            echo '<pre>';print_r($fileName);exit;
        ...
        try {
            if ($target->save()) {
                if (!empty($request->group_id) && in_array($request->group_id, [3, 4])) {
                    $userBpro = UserBasicProfile::select('id')->where('user_id', $id)->first();

                    $newTarget = !empty($userBpro->id) ? UserBasicProfile::find($userBpro->id) : new UserBasicProfile;
                    $newTarget->user_id = $id;
                    $newTarget->arms_service_id = $request->arms_service_id;
                    $newTarget->father_name = $request->father_name;
                    $newTarget->commission_type = $request->commission_type;
                    $newTarget->commissioning_course_id = $request->commissioning_course_id;
    ...
    public function destroy(Request $request, $id) {
        ...
        $dependencyArr = [
            //administratives dependancyArr
            'User' => ['1' => 'created_by', '2' => 'updated_by'],
            'DsGroupMemberTemplate' => ['1' => 'user_id'],
            'EventAssessmentMarking' => ['1' => 'updated_by'],
            'CiModerationMarking' => ['1' => 'updated_by'],
            'ComdtObsnMarking' => ['1' => 'updated_by'],
        ];
        ...
        $groupId = !empty($target->group_id) ? $target->group_id : 0;

        DB::beginTransaction();
        try {
            if ($target->delete()) {
                if (in_array($groupId, [3, 4])) {

                    $userBpro = UserBasicProfile::select('id')->where('user_id', $id)->first();
                    $uOtr = UserOthers::select('id')->where('user_basic_profile_id', $userBpro->id)->first();
                    $uCnVst = UserCountryVisit::select('id')->where('user_basic_profile_id', $userBpro->id)->first();
                    $uPerAdr = UserPermanentAddress::select('id')->where('user_basic_profile_id', $userBpro->id)->first();
                    $uCvlEdu = UserCivilEducation::select('id')->where('user_basic_profile_id', $userBpro->id)->first();
                    $uSerRec = UserServiceRecord::select('id')->where('user_basic_profile_id', $userBpro->id)->first();
                    $uRelDef = UserRelativeInDefence::select('id')->where('user_basic_profile_id', $userBpro->id)->first();
                    $uChi = UserChild::select('id')->where('user_basic_profile_id', $userBpro->id)->first();
                    $uPreAdr = UserPresentAddress::select('id')->where('user_basic_profile_id', $userBpro->id)->first();
                    $uPas = UserPassport::select('id')->where('user_basic_profile_id', $userBpro->id)->first();
                    $uMsn = UserMission::select('id')->where('user_basic_profile_id', $userBpro->id)->first();
                    $uBnk = UserBank::select('id')->where('user_basic_profile_id', $userBpro->id)->first();

                    if (!empty($userBpro->id)) {
                        $target1 = UserBasicProfile::find($userBpro->id);
                        $target1->delete();
                    }

                    if (!empty($uOtr->id)) {
                        $target1 = UserOthers::find($uOtr->id);
                        $target1->delete();
                    }

                    if (!empty($uCnVst->id)) {
                        $target1 = UserCountryVisit::find($uCnVst->id);
                        $target1->delete();
                    }

                    if (!empty($uPerAdr->id)) {
                        $target1 = UserPermanentAddress::find($uPerAdr->id);
                        $target1->delete();
                    }

                    if (!empty($uCvlEdu->id)) {
                        $target1 = UserCivilEducation::find($uCvlEdu->id);
                        $target1->delete();
                    }

                    if (!empty($uSerRec->id)) {
                        $target1 = UserServiceRecord::find($uSerRec->id);
                        $target1->delete();
                    }

                    if (!empty($uRelDef->id)) {
                        $target1 = UserRelativeInDefence::find($uRelDef->id);
                        $target1->delete();
                    }

                    if (!empty($uChi->id)) {
                        $target1 = UserChild::find($uChi->id);
                        $target1->delete();
                    }

                    if (!empty($uPreAdr->id)) {
                        $target1 = UserPresentAddress::find($uPreAdr->id);
                        $target1->delete();
                    }

                    if (!empty($uPas->id)) {
                        $target1 = UserPassport::find($uPas->id);
                        $target1->delete();
                    }

                    if (!empty($uMsn->id)) {
                        $target1 = UserMission::find($uMsn->id);
                        $target1->delete();
                    }

                    if (!empty($uBnk->id)) {
                        $target1 = UserBank::find($uBnk->id);
                        $target1->delete();
                    }
                }
            }
...
//                        , 'users.photo', 'users.phone', 'users.full_name', 'users.personal_no'
//                        , 'users.official_name'
//                        , DB::raw("CONCAT(rank.code, ' ', users.full_name, ' (', users.official_name, ')') as user_name"), 'user_basic_profile.commissioning_course_id as commissioning_course_id', 'user_basic_profile.commission_type as commissioning_type'
...
//        echo '<pre>';        print_r($userInfoData->toArray()); exit;
        ...
        $passportInfoData = UserPassport::where('user_basic_profile_id', !empty($userInfoData->id) ? $userInfoData->id : 0)
                ->select('id', 'user_basic_profile_id', 'passport_no', 'place_of_issue', 'date_of_issue'
    ...
    public function updateProfile(Request $request) {
        ...
        if ($request->update_id == 1) {
            ...
            $rules = [
                ...
                'rank_id' => 'required|not_in:0',
                'appointment_id' => 'required|not_in:0',
                'personal_no' => 'required|unique:users,personal_no,' . $id . ',id,group_id,' . $target->group_id,
                'full_name' => 'required',
                'official_name' => 'required',
                'username' => 'required|alpha_dash|unique:users,username,' . $id
            ];
...
//                    $rules['mil_qual.' . $key . '.result'] = 'required|not_in:0';
//                    $messages['mil_qual.' . $key . '.result.not_in'] = __('label.RESULT_FIELD_EMPTY_MESSAGE', ["counter" => $row]);
                }
    ...
    public function updateMedicalDetails(Request $request) {
        ...
        $passportInfo = UserPassport::select('id', 'pass_scan_copy', 'pass_scan_copy_name', 'photo_without_uniform')
                ->where('user_basic_profile_id', $request->user_basic_profile_id)->first();
        if (!empty($request->pass_scan_copy) && !empty($passportInfo->pass_scan_copy)) {
        ...
        if (!empty($request->photo_without_uniform) && !empty($passportInfo->photo_without_uniform)) {
            $prevfilePName = 'public/uploads/userPassPhoto/' . $passportInfo->photo_without_uniform;
        ...
        if (!empty($fileP)) {
            ...
            $uploadSuccess = $fileP->move('public/uploads/userPassPhoto', $fileNameP);
//            echo '<pre>';print_r($fileName);exit;
        }
...
//        $previousFileName = $target->photo;
        ...
        $rules = [
            'wing_id' => 'required|not_in:0',
            'rank_id' => 'required|not_in:0',
            'personal_no' => 'required|unique:users,personal_no,' . $userId . ',id,group_id,' . $target->group_id,
        ...
        $messages = array(
            'bn_name.required' => __('label.FULL_NAME_BANGLA_FIELD_IS_REQUIRED'),
        );
        ...
        try {
            if ($target->save()) {
                ...
                $newTarget->user_id = $target->id;
                $newTarget->bn_name = $request->bn_name;
                $newTarget->arms_service_id = $request->arms_service_id;
                $newTarget->father_name = $request->father_name;
                $newTarget->commission_type = $request->commission_type;
                $newTarget->commissioning_course_id = $request->commissioning_course_id;
