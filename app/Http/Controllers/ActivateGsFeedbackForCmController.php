...
class ActivateGsFeedbackForCmController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $detailsArr = $statArr = [];
        $statValue = '';
        $statusArr = ActivateGsFeedbackForCm::select('lesson_id', 'subject_id', 'gs_id', 'status')->get();
        ...
        return view('activateGsFeedbackForCm.index')->with(compact('activeTrainingYearInfo', 'courseList', 'request', 'targetArr', 'statArr'));
    }
    ...
    public function setStat(Request $request)
    {
        ...
        try {
            $prevActDeactInfo = ActivateGsFeedbackForCm::where('course_id', $courseId)
                ->where('lesson_id', $lessonId)
                ->where('subject_id', $subjectId)->where('gs_id', $gsId)
                ->select('id')->first();

            $actDeact = !empty($prevActDeactInfo->id) ? ActivateGsFeedbackForCm::find($prevActDeactInfo->id) : new ActivateGsFeedbackForCm;
