...
class OtherInfoWiseCmAnalyticsController extends Controller {
...
//            'personal_no' => __('label.PERSONAL_NO'),
        ...
        if ($request->generate == 'true') {
            ...
            $cmArr = CmOthers::leftJoin('cm_basic_profile', 'cm_basic_profile.id', 'cm_others.cm_basic_profile_id')
            ...
            $extra_cur = $request->extra_cur;
            if (!empty($request->extra_cur)) {
                $cmArr->where(function($query) use ($extra_cur) {
                    $query->where('cm_others.extra_curriclar_expt', 'LIKE', '%' . $extra_cur . '%');
                });
            }

            $award = $request->award;
            if (!empty($request->award)) {
                $cmArr->where(function($query) use ($award) {
                    $query->where('cm_others.decoration_id', 'LIKE', '%' . $award . '%');
                });
            }
            ...
            $cmArr = $cmArr->select('cm_basic_profile.personal_no', 'cm_basic_profile.full_name', 'cm_basic_profile.official_name'
                            ...
                            , 'cm_others.decoration_id', 'cm_others.extra_curriclar_expt', 'arms_service.code as arms_service_name', 'cm_basic_profile.religion_id', 'cm_basic_profile.gender')
                    ->orderBy('course.id', 'desc');
            ...
            $fileName = 'Other_Info_Wise_CM_Analytics';
            $fileName = Common::getFileFormatedName($fileName);
        }


        if ($request->view == 'print') {
            return view('cmAnalytics.otherInfo.print.index')->with(compact('request', 'courseList'
                                    , 'targetArr', 'qpArr', 'sortByList'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('cmAnalytics.otherInfo.print.index', compact('request', 'activeTrainingYearList', 'courseList', 'termList'
                                    , 'targetArr', 'qpArr', 'sortByList'))
            ...
            return Excel::download(new ExcelExport('cmAnalytics.otherInfo.print.index', compact('request', 'courseList'
                                    , 'targetArr', 'qpArr', 'sortByList'), 3), $fileName . '.xlsx');
        } else {

            return view('cmAnalytics.otherInfo.index', compact('request', 'courseList', 'nameArr','printOptionList', 'columnArr'
    ...
    public function filter(Request $request) {
        ...
        $url = 'course_id=' . $implodeCourses . '&name=' . urlencode($request->name) . '&wing_id=' . $request->wing_id . '&rank_id=' . $request->rank_id . '&arms_service_id=' . $request->arms_service_id . '&extra_cur=' . $request->extra_cur . '&award=' . $request->award
                . '&sort=' . $request->sort;
        ...
        return redirect('otherInfoWiseCmAnalytics?generate=true&' . $url);
    }
