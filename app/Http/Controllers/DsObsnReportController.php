...
class DsObsnReportController extends Controller {

    private $controller = 'DsObsnReport';
...
//            exit;
        ...
        if ($request->view == 'print') {
            return view('report.dsObsn.print.index')->with(compact('activeTrainingYearList', 'courseList', 'termList'
            ...
            $pdf = PDF::loadView('report.dsObsn.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'dsMksWtArr', 'dsDataList'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr', 'assignedDsObsnInfo'))
            ...
            return Excel::download(new ExcelExport('report.dsObsn.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'dsMksWtArr', 'dsDataList'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr', 'assignedDsObsnInfo')), $fileName . '.xlsx');
        }

        return view('report.dsObsn.index', compact('activeTrainingYearList', 'courseList', 'termList'
                        , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'dsMksWtArr', 'dsDataList'
                        , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr', 'assignedDsObsnInfo'));
    }
    ...
    public function getCourse(Request $request) {
        ...
        $courseList = Course::where('training_year_id', $request->training_year_id)
                ->where('status', '<>', '0')->orderBy('training_year_id', 'desc')
                ->orderBy('id', 'desc')->pluck('name', 'id')
                ->toArray();

        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + $courseList;

        $html = view('report.dsObsn.getCourse', compact('courseList'))->render();
        $html1 = view('report.dsObsn.getCourseErr', compact('courseList'))->render();
    ...
    public function getTerm(Request $request) {
        ...
        $html = view('report.dsObsn.getTerm', compact('termList'))->render();
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('dsObsnReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('dsObsnReport?generate=true&' . $url);
    }
