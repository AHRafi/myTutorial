...
class ComdtObsnMarkingController extends Controller {
    ...
    public function index(Request $request) {
        ...
        if (empty($activeTrainingYearInfo)) {
            $void['header'] = __('label.COMDT_OBSN_MARKING');
...
//                        ->where('status', '1')->orderBy('id', 'desc')->pluck('name', 'id')->toArray();
        ...
        $assignedObsnLimitInfo = CiComdtObsnMarkingLimit::select('comdt_mks_limit', 'comdt_limit_percent')
                        ->where('course_id', $activeCourse->id)->first();

        // get assigned ci obsn wt
        $assignedObsnInfo = CriteriaWiseWt::select('ci_obsn_wt', 'comdt_obsn_wt')->where('course_id', $activeCourse->id)->first();
...
//term wise total
        ...
        if (!empty($termDataArr)) {
            foreach ($termDataArr as $termId => $termName) {
                ...
                $eventMksWtArr['after_comdt_obsn'] = $eventMksWtArr['after_ci_obsn'] + (!empty($assignedObsnInfo->comdt_obsn_wt) ? $assignedObsnInfo->comdt_obsn_wt : 0);
            }
        }
            ...
            foreach ($cmArr as $cmId => $cmInfo) {
                if (!empty($termDataArr)) {
                    foreach ($termDataArr as $termId => $termName) {
                        ...
                        $cmArr[$cmId]['after_comdt_obsn'] = $cmArr[$cmId]['after_ci_obsn'];
                    }
                }
            }
        }
...
//        exit;
        ...
        $ciObsnDataArr = CiObsnMarking::join('ci_obsn_marking_lock', function($join) {
                    $join->on('ci_obsn_marking_lock.course_id', 'ci_obsn_marking.course_id');
                    $join->on('ci_obsn_marking_lock.locked_by', 'ci_obsn_marking.updated_by');
                })
                ...
                ->select('ci_obsn_marking.cm_id', 'ci_obsn_marking.ci_obsn', 'ci_obsn_marking.wt'
                        ...
                        , 'grading_system.id as grade_id', 'ci_obsn_marking.ci_obsn_mks')
        ...
        if (!$ciObsnDataArr->isEmpty()) {
            foreach ($ciObsnDataArr as $ciObsnData) {
                ...
                $cmArr[$ciObsnData->cm_id]['after_comdt_obsn'] = $cmArr[$ciObsnData->cm_id]['after_ci_obsn'];
            }
        }

        // get position after ci obsn
        // get previous data
        $comdtObsnDataArr = ComdtObsnMarking::leftJoin('grading_system', 'grading_system.id', 'comdt_obsn_marking.grade_id')
                ->select('comdt_obsn_marking.cm_id', 'comdt_obsn_marking.comdt_obsn', 'comdt_obsn_marking.wt'
                        ...
                        , 'grading_system.id as grade_id', 'comdt_obsn_marking.comdt_obsn_mks')
                ->where('comdt_obsn_marking.course_id', $activeCourse->id)
                ->get();

        if (!$comdtObsnDataArr->isEmpty()) {
            foreach ($comdtObsnDataArr as $comdtObsnData) {
                $cmArr[$comdtObsnData->cm_id]['comdt_obsn_mks'] = $comdtObsnData->comdt_obsn_mks ?? '';
                ...
                $cmArr[$comdtObsnData->cm_id]['after_comdt_obsn'] = $cmArr[$comdtObsnData->cm_id]['after_ci_obsn'] + (!empty($assignedObsnInfo->comdt_obsn_wt) && !empty($comdtObsnData->comdt_obsn) ? $assignedObsnInfo->comdt_obsn_wt : 0);
            }
        }
        ...
        if (!empty($cmArr)) {
            foreach ($cmArr as $cmId => $cmInfo) {
                ...
                $afterComdtObsnTotalWt = !empty($cmInfo['after_comdt_obsn']) ? $cmInfo['after_comdt_obsn'] : 0;
                ...
                $cmArr[$cmId]['total_wt_after_comdt'] = $cmArr[$cmId]['total_wt'] + $comdtWt;
                
                $cmArr[$cmId]['percent_after_comdt'] = 0;
                if (!empty($afterComdtObsnTotalWt)) {
                    $cmArr[$cmId]['percent_after_comdt'] = ($cmArr[$cmId]['total_wt_after_comdt'] / $afterComdtObsnTotalWt) * 100;
                }
            }
        }
        ...
        $cmArr = Common::getPosition($cmArr, 'percent_after_comdt', 'position_after_comdt_obsn');

        $cmArr = Common::getGradeName($cmArr, $gradeInfo, 'percent_after_comdt', 'grade_after_comdt');
        ...
        // get comdt lock info
        $comdtObsnLockInfo = ComdtObsnMarkingLock::select('status')->where('course_id', $activeCourse->id)->first();
        $autoSave = !empty($comdtObsnLockInfo) ? 0 : 1;
        return view('comdtObsnMarking.index')->with(compact('activeTrainingYearInfo', 'activeCourse', 'sortByList'
                                , 'assignedObsnInfo', 'gradeInfo', 'eventMksWtArr', 'cmArr', 'ciObsnLockInfo'
                                , 'termDataArr', 'autoSave', 'comdtObsnLockInfo', 'ciObsnDataArr'
                                , 'comdtObsnDataArr', 'assignedObsnLimitInfo'));
    }
            ...
            foreach ($cmArr as $cmId => $cmInfo) {
                if (!empty($termDataArr)) {
                    foreach ($termDataArr as $termId => $termName) {
                        ...
                        $cmArr[$cmId]['after_comdt_obsn'] = $cmArr[$cmId]['after_ci_obsn'] + (!empty($assignedObsnInfo->comdt_obsn_wt) ? $assignedObsnInfo->comdt_obsn_wt : 0);
                    }
                }
            }
        }
...
//        exit;
        ...
        $html = view('comdtObsnMarking.showCmMarkingList', compact('activeTrainingYearInfo', 'activeCourse', 'sortByList'
                        , 'assignedObsnInfo', 'gradeInfo', 'eventMksWtArr', 'cmArr', 'ciObsnLockInfo'
                        , 'termDataArr', 'autoSave', 'comdtObsnLockInfo', 'ciObsnDataArr'
                        , 'comdtObsnDataArr', 'assignedObsnLimitInfo'))->render();
        ...
        if (!empty($request->wt)) {
            foreach ($request->wt as $key => $wtInfo) {
                if ($request->data_id == '2') {
                    $rules['wt.' . $key . '.comdt_obsn'] = 'required';
                    $message['wt.' . $key . '.comdt_obsn' . '.required'] = __('label.WT_FIELD_IS_REQUIRED_FOR', ['cm_name' => $cmName[$key]]);
                }
                
                $wt = !empty($wtInfo['comdt_obsn']) ? $wtInfo['comdt_obsn'] : 0;
                $sum += $wt;
            }
        }
...
// End validation
        ...
        if (!empty($request->wt)) {
            foreach ($request->wt as $cmId => $wtInfo) {
                ...
                $data[$i]['cm_id'] = $cmId ?? 0;
                $data[$i]['comdt_obsn_mks'] = $wtInfo['comdt_obsn_mks'] ?? null;
                $data[$i]['comdt_obsn'] = $wtInfo['comdt_obsn'] ?? null;
...
// Save data
        ...
        try {
            ComdtObsnMarking::where('course_id', $request->course_id)->delete();
            if (ComdtObsnMarking::insert($data)) {
                $successMsg = __('label.OBSN_WT_HAS_BEEN_ASSIGNED_SUCCESSFULLY');
                $errorMsg = __('label.OBSN_WT_CUOLD_NOT_BE_ASSIGNED');

                if ($request->data_id == '2') {
                    $target = new ComdtObsnMarkingLock;
    ...
    public function getRequestForUnlockModal(Request $request) {
        $view = view('comdtObsnMarking.showRequestForUnlockModal')->render();
...
// validation
        ...
        $comdtObsnLockInfo = ComdtObsnMarkingLock::select('id')->where('course_id', $request->course_id)->first();

        if (!empty($comdtObsnLockInfo)) {
            $target = ComdtObsnMarkingLock::where('id', $comdtObsnLockInfo->id)
                    ->update([
                'status' => '2',
            ...
            if ($target) {
                ...
                return Response::json(array('success' => false, 'message' => __('label.REQUEST_FOR_UNLOCK_COULD_NOT_BE_SENT_TO_SUPERADMIN')), 401);
            }
        }
    }

    public function requestCourseSatatusSummary(Request $request) {
        $loadView = 'comdtObsnMarking.showCourseStatusSummary';
        return Common::requestCourseSatatusSummary($request, $loadView);
    }

    public function getDsMarkingSummary(Request $request) {
        $loadView = 'comdtObsnMarking.showDsMarkingSummaryModal';
...
//        echo '<pre>';        print_r($request->all()); exit;
        ...
        $target = ComdtObsnMarking::where('course_id', $request->course_id)
