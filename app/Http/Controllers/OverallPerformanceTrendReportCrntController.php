...
class OverallPerformanceTrendReportCrntController extends Controller {

    private $controller = 'OverallPerformanceTrendReportCrnt';
    ...
    public function index(Request $request) {
        ...
        if (empty($activeTrainingYearList)) {
            $void['header'] = __('label.OVERALL_PERFORMANCE_TREND');
        ...
        $gradeInfo = $comdtObsnLockInfo = $ciObsnLockInfo = $maxCm = 0;
        $assignedObsnInfo = $assignedDsObsnArr = $eventMksWtArr = $eventWiseMksArr = $overallMksArr = $gradeList = [];
...
//            echo '<pre>';print_r($overallMksArr);
//            echo '<pre>';print_r($maxCm);exit;
        }

        return view('reportCrnt.overallPerformanceTrend.index', compact('activeTrainingYearList'
                        , 'courseList', 'cmArr', 'maxCm'
                        , 'assignedObsnInfo', 'gradeInfo', 'request'
                        , 'gradeList', 'overallMksArr'));
    }
    ...
    public function getCourse(Request $request) {
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + Course::where('training_year_id', $request->training_year_id)
                        ...
                        ->pluck('name', 'id')
                        ->toArray();
        $html = view('reportCrnt.overallPerformanceTrend.getCourse', compact('courseList'))->render();
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('overallPerformanceTrendReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('overallPerformanceTrendReportCrnt?generate=true&' . $url);
    }
