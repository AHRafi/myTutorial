...
class EventAvgTrendReportCrntController extends Controller {

    private $controller = 'EventAvgTrendReportCrnt';
    ...
    public function index(Request $request) {
        ...
        $cmArr = CmBasicProfile::where('cm_basic_profile.course_id', $courseList->id)
        ...
        $eventMksWtArr = $eventWiseMksArr = $eventAvgMksArr = $cmMksMinMaxArr = $eventIds = $selectedEvents = [];
        if ($request->generate == 'true') {
            ...
            if (!empty($cmWiseMksArr)) {
                foreach ($cmWiseMksArr as $cmId => $eventInfo) {
                    foreach ($eventInfo as $eventId => $mksInfo) {
                        ...
                        //cm wise event mks %
                        $cmMksMinMaxArr[$eventId][$cmId] = $eventPercentage;
                        $cmMksMinMaxArr['grand_max_min'][] = $eventPercentage;

                        $eventAvgMksArr[$eventId]['total_mks_percent'] = !empty($eventAvgMksArr[$eventId]['total_mks_percent']) ? $eventAvgMksArr[$eventId]['total_mks_percent'] : 0;
                        $eventAvgMksArr[$eventId]['total_mks_percent'] += $eventPercentage;
                    }
                }
            }
            ...
            if (!empty($eventAvgMksArr)) {
                foreach ($eventAvgMksArr as $eventId => $mksInfo) {
                    $eventMks = !empty($mksInfo['total_mks_percent']) ? $mksInfo['total_mks_percent'] : 0;
                    $eventAvgMksArr[$eventId]['mks_percent'] = ($eventMks) / (!empty($cmArr) ? sizeof($cmArr) : 1);

                    //Event Avg Max/Min 
                    $eventAvgMksArr[$eventId]['max'] = !empty($cmMksMinMaxArr[$eventId]) ? max($cmMksMinMaxArr[$eventId]) : 0;
                    $eventAvgMksArr[$eventId]['min'] = !empty($cmMksMinMaxArr[$eventId]) ? min($cmMksMinMaxArr[$eventId]) : 0;
                    $eventAvgMksArr['grand_max_min']['max'] = !empty($cmMksMinMaxArr['grand_max_min']) ? max($cmMksMinMaxArr['grand_max_min']) + (max($cmMksMinMaxArr['grand_max_min']) + 10 < 100 ? 10 : 0) : 0;
                    $eventAvgMksArr['grand_max_min']['min'] = !empty($cmMksMinMaxArr['grand_max_min']) ? min($cmMksMinMaxArr['grand_max_min']) - (min($cmMksMinMaxArr['grand_max_min']) - 10 > 0 ? 10 : 0) : 0;
                }
            }


//            echo '<pre>';
//            print_r($eventAvgMksArr);
//            exit;
        }


        return view('reportCrnt.eventAvgTrend.index', compact('activeTrainingYearList', 'courseList', 'termList'
                        , 'eventList', 'eventIds', 'eventAvgMksArr', 'selectedEvents'));
    }
    ...
    public function getCourse(Request $request) {
        ...
        $showEventView = view('reportCrnt.eventAvgTrend.getCourseWiseEvent', compact('eventList'))->render();
        $html = view('reportCrnt.eventAvgTrend.getCourse', compact('courseList'))->render();
        $html2 = view('reportCrnt.eventAvgTrend.getTerm', compact('termList'))->render();
    ...
    public function getTerm(Request $request) {
        ...
        $eventList = [];
        $html = view('reportCrnt.eventAvgTrend.getTerm', compact('termList'))->render();
        $showEventView = view('reportCrnt.eventAvgTrend.getCourseWiseEvent', compact('eventList'))->render();

        return response()->json(['html' => $html, 'showEventView' => $showEventView]);
    }

    public function getCourseWiseEvent(Request $request) {
        $eventList = TermToEvent::join('event', 'event.id', '=', 'term_to_event.event_id')
                ->where('term_to_event.course_id', $request->course_id);
        if (!empty($request->term_id)) {
            $eventList = $eventList->where('term_to_event.term_id', $request->term_id);
        ...
        $showEventView = view('reportCrnt.armsServiceWiseEventTrend.getCourseWiseEvent', compact('eventList'))->render();
    ...
    public function filter(Request $request) {
        ...
        $url = 'training_year_id=' . $request->training_year_id . '&course_id=' . $request->course_id
                . '&term_id=' . $request->term_id . '&event_id=' . $eventIds
                . '&range_start=' . $request->range_start . '&range_end=' . $request->range_end;

        $validator = Validator::make($request->all(), $rules, $messages);
        if ($validator->fails()) {
            return redirect('eventAvgTrendReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('eventAvgTrendReportCrnt?generate=true&' . $url);
    }
