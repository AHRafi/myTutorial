...
class CourseWiseDocReportController extends Controller
{
     public function index(Request $request) {
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + Course::orderBy('id', 'desc')->pluck('name', 'id')->toArray();
        ...
        if ($request->generate == 'true') {
            ...
            $fileName = 'Course_Wise_Document_Report';
            $fileName = Common::getFileFormatedName($fileName);
        }
        if ($request->view == 'print') {
            return view('referenceArchive.courseWiseDocReport.print.index')->with(compact('request', 'courseList', 'contentModuleList', 'targetArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('referenceArchive.courseWiseDocReport.print.index', compact('request', 'courseList', 'contentModuleList', 'targetArr'))
                    ->setPaper('a4', 'landscape')
                    ->setOptions(['defaultFont' => 'sans-serif']);

            return $pdf->download($fileName . '.pdf');
        } elseif ($request->view == 'excel') {
            return Excel::download(new ExcelExport('referenceArchive.courseWiseDocReport.print.index', compact('request'
                                    , 'courseList', 'contentModuleList', 'targetArr'), 3), $fileName . '.xlsx');
        } else {
         return view('referenceArchive.courseWiseDocReport.index',compact('request','courseList', 'contentModuleList', 'contentDetailsInfoArr', 'targetArr'));
     }
     }
...
//        print_r($request->all());
        ...
        if ($validator->fails()) {
            return redirect('courseWiseDocReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }

        return redirect('courseWiseDocReport?generate=true&' . $url);
    }
