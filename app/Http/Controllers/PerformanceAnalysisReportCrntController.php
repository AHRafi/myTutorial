...
class PerformanceAnalysisReportCrntController extends Controller {

    private $controller = 'PerformanceAnalysisReportCrnt';
    ...
    public function index(Request $request) {
        ...
        if (empty($activeTrainingYearList)) {
            $void['header'] = __('label.PERFORMANCE_ANALYSIS');
        ...
        $eventMksWtArr = $cmArr = $rowSpanArr = $achieveMksWtArr = $eventWiseMksWtArr = [];
        if ($request->generate == 'true') {

            $selectedCmId = !empty($request->cm_id) ? explode(",", $request->cm_id) : [];
            ...
            $fileName = 'Performance_Analysis' . $tyName . $courseName;
...
//            if (!empty($selectedCmId)) {
//                $cmDataArr = $cmDataArr->whereIn('cm_basic_profile.id', $selectedCmId);
//            }
//            $cmDataArr = $cmDataArr->select('cm_basic_profile.id', 'cm_basic_profile.photo', 'cm_basic_profile.personal_no'
...
//            }
            $cmDataArr = $cmDataArr->where('cm_basic_profile.status', '1')
                    ->whereIn('cm_basic_profile.id', $selectedCmId)
            ...
            if (!empty($selectedCmId)) {
                $eventWiseMksWtInfo = $eventWiseMksWtInfo->whereIn('event_assessment_marking.cm_id', $selectedCmId);
            }
            $eventWiseMksWtInfo = $eventWiseMksWtInfo->get();
...
// ci moderation wise mks & wt 
            ...
            if (!empty($selectedCmId)) {
                $ciModWiseMksWtInfo = $ciModWiseMksWtInfo->whereIn('ci_moderation_marking.cm_id', $selectedCmId);
            }
            $ciModWiseMksWtInfo = $ciModWiseMksWtInfo->get();
...
// comdt moderation wise mks & wt 
            ...
            if (!empty($selectedCmId)) {
                $comdtModWiseMksWtInfo = $comdtModWiseMksWtInfo->whereIn('comdt_moderation_marking.cm_id', $selectedCmId);
            }
            $comdtModWiseMksWtInfo = $comdtModWiseMksWtInfo->get();
...
//        exit;
        ...
        if ($request->view == 'print') {
            return view('reportCrnt.performanceAnalysis.print.index')->with(compact('activeTrainingYearList', 'courseList'
                                    , 'cmList', 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'request'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('reportCrnt.performanceAnalysis.print.index', compact('activeTrainingYearList', 'courseList'
                                    , 'cmList', 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'request'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'))
            ...
            return Excel::download(new ExcelExport('reportCrnt.performanceAnalysis.print.index', compact('activeTrainingYearList', 'courseList'
                                    , 'cmList', 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'request'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr')), $fileName . '.xlsx');
        }

        return view('reportCrnt.performanceAnalysis.index', compact('activeTrainingYearList', 'courseList'
                        , 'cmList', 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'request'
                        , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'));
    }
    ...
    public function getCourse(Request $request) {
        ...
        $html1 = view('reportCrnt.performanceAnalysis.getCm', compact('cmList'))->render();
        $html = view('reportCrnt.performanceAnalysis.getCourse', compact('courseList'))->render();
    ...
    public function getCm(Request $request) {
        ...
        $html = view('reportCrnt.performanceAnalysis.getCm', compact('cmList'))->render();
        return Response::json(['html' => $html]);
    }

    public function filter(Request $request) {
//        echo '<pre>';            print_r($request->all()); exit;  
        $cmId = !empty($request->cm_id) ? implode(",", $request->cm_id) : '';
        $messages = [];
        $rules = [
        ...
        $url = 'training_year_id=' . $request->training_year_id . '&course_id=' . $request->course_id . '&cm_id=' . $cmId . '&sort=' . $request->sort;

        $validator = Validator::make($request->all(), $rules, $messages);
        if ($validator->fails()) {
            return redirect('performanceAnalysisReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('performanceAnalysisReportCrnt?generate=true&' . $url);
    }
