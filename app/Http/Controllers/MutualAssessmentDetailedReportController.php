...
class MutualAssessmentDetailedReportController extends Controller {

    private $controller = 'MutualAssessmentDetailedReport';
    ...
    public function index(Request $request) {
        ...
        if ($request->view == 'print') {
            return view('report.mutualAssessmentDetailed.print.index')->with(compact('activeTrainingYearList', 'courseList', 'termList'
            ...
            $pdf = PDF::loadView('report.mutualAssessmentDetailed.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'factorList', 'synList', 'subSynList', 'hasSubSyn', 'cmArr', 'markingPositionArr'
                                    , 'totalPositionArr', 'markingCmArr', 'sortByList', 'eventGroupList'
                                    , 'maProcess', 'eventList', 'subEventList', 'subSubEventList', 'subSubSubEventList'))
            ...
            return Excel::download(new ExcelExport('report.mutualAssessmentDetailed.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'factorList', 'synList', 'subSynList', 'hasSubSyn', 'cmArr', 'markingPositionArr'
                                    , 'totalPositionArr', 'markingCmArr', 'sortByList', 'eventGroupList'
                                    , 'maProcess', 'eventList', 'subEventList', 'subSubEventList', 'subSubSubEventList')), $fileName . '.xlsx');
        }

        return view('report.mutualAssessmentDetailed.index', compact('activeTrainingYearList', 'courseList', 'termList'
                        , 'factorList', 'synList', 'subSynList', 'hasSubSyn', 'cmArr', 'markingPositionArr'
                        , 'totalPositionArr', 'markingCmArr', 'sortByList', 'eventGroupList'
                        , 'maProcess', 'eventList', 'subEventList', 'subSubEventList', 'subSubSubEventList'));
    }
    ...
    public function getCourse(Request $request) {
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + Course::where('training_year_id', $request->training_year_id)
                        ...
                        ->pluck('name', 'id')
                        ->toArray();
        $html = view('report.mutualAssessmentDetailed.getCourse', compact('courseList'))->render();
    ...
    public function getTerm(Request $request) {
        ...
        $html = view('report.mutualAssessmentDetailed.getTerm', compact('termList'))->render();
        return Response::json(['html' => $html]);
    }
    ...
    public function getSubEvent(Request $request) {
        ...
        $html2 = view('report.mutualAssessmentDetailed.showSubSubEvent', compact('subSubEventList'))->render();
        $html3 = view('report.mutualAssessmentDetailed.showSubSubSubEvent', compact('subSubSubEventList'))->render();

        $html1 = Self::getEventGroups($request);
        $html = view('report.mutualAssessmentDetailed.showSubEvent', compact('subEventList'))->render();
    ...
    public function getSubSubEvent(Request $request) {
        ...
        $subSubSubEventList = ['0' => __('label.SELECT_SUB_SUB_SUB_EVENT_OPT')];
        $html2 = view('report.mutualAssessmentDetailed.showSubSubSubEvent', compact('subSubSubEventList'))->render();


        $html1 = Self::getEventGroups($request);
        $html = view('report.mutualAssessmentDetailed.showSubSubEvent', compact('subSubEventList'))->render();
    ...
    public function getSubSubSubEvent(Request $request) {
        ...
        $html1 = Self::getEventGroups($request);
        $html = view('report.mutualAssessmentDetailed.showSubSubSubEvent', compact('subSubSubEventList'))->render();
    ...
    public function getEventGroups(Request $request) {
        ...
        $html = view('report.mutualAssessmentDetailed.showEventGroup', compact('eventGroupList'))->render();
    ...
    public function getSynOrGp(Request $request) {
        ...
        $html1 = view('report.mutualAssessmentDetailed.showSubEvent', compact('subEventList'))->render();
        $html2 = view('report.mutualAssessmentDetailed.showSubSubEvent', compact('subSubEventList'))->render();
        $html3 = view('report.mutualAssessmentDetailed.showSubSubSubEvent', compact('subSubSubEventList'))->render();
        $html4 = view('report.mutualAssessmentDetailed.showEventGroup', compact('eventGroupList'))->render();

        $html = view('report.mutualAssessmentDetailed.getSyn', compact('synList', 'subSynList', 'eventList', 'maProcess'))->render();
    ...
    public function getsubSyn(Request $request) {
        ...
        if (sizeof($subSynList) > 1) {
            $html = view('report.mutualAssessmentDetailed.getSubSyn', compact('subSynList'))->render();
        }
        return Response::json(['html' => $html]);
    }
...
//            $messages['event_group_id.not_in'] = __('label.EVENT_GROUP_MUST_BE_SELECTED');
        ...
        if ($validator->fails()) {
            return redirect('mutualAssessmentDetailedReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('mutualAssessmentDetailedReport?generate=true&' . $url);
    }
