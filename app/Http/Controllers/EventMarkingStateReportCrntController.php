...
class EventMarkingStateReportCrntController extends Controller {

    private $controller = 'EventMarkingStateReportCrnt';
...
//            $sortByList = ['position' => __('label.POSITION')] + $sortByList;
        ...
        if ($request->generate == 'true') {
            ...
            $fileName = 'Event_Marking_State_Report' . $tyName . $courseName . $eventName;
...
//            echo '<pre>';            print_r($eventInfo->toArray()); exit;
            ...
            $subEventInfo = $subEventInfo->where('event_to_sub_event.avg_marking', '1')->where('sub_event.status', '1')
            ...
            $subSubEventInfo = $subSubEventInfo->where('event_to_sub_event.avg_marking', '1')->where('sub_sub_event.status', '1')
            ...
            $subSubSubEventInfo = $subSubSubEventInfo->where('event_to_sub_event.avg_marking', '1')->where('sub_sub_sub_event.status', '1')
...
// comdt moderation wise mks & wt 
            ...
            if (!empty($cmArr)) {
                foreach ($cmArr as $cmId => $cmInfo) {
                            ...
                            foreach ($subEvInfo as $subEventId => $subSubEvInfo) {
                                ...
                                foreach ($subSubEvInfo as $subSubEventId => $subSubSubEvInfo) {
                                    foreach ($subSubSubEvInfo as $subSubSubEventId => $info) {
                                        ...
                                        $cmArr[$cmId]['total_mked'] = !empty($cmArr[$cmId]['total_mked']) ? $cmArr[$cmId]['total_mked'] : 0;
                                        $cmArr[$cmId]['total_mked'] += (!empty($TotalTermMks) ? 1 : 0);
                    ...
                    // total no. of none paticipant cm
                    $eventMksWtArr['total_none_pct_cm'] = !empty($eventMksWtArr['total_none_pct_cm']) ? $eventMksWtArr['total_none_pct_cm'] : 0;
                    $eventMksWtArr['total_none_pct_cm'] += (!empty($cmArr[$cmId]['total_mked']) ? 0 : 1);
                }
            }
...
//            exit;
        ...
        if ($request->view == 'print') {
            return view('reportCrnt.eventMarkingState.print.index')->with(compact('activeTrainingYearList', 'courseList'
                                    , 'eventList', 'sortByList', 'assignedObsnInfo', 'gradeInfo'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('reportCrnt.eventMarkingState.print.index', compact('activeTrainingYearList', 'courseList'
                                    , 'eventList', 'sortByList', 'assignedObsnInfo', 'gradeInfo'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'))
            ...
            return Excel::download(new ExcelExport('reportCrnt.eventMarkingState.print.index', compact('activeTrainingYearList', 'courseList'
                                    , 'eventList', 'sortByList', 'assignedObsnInfo', 'gradeInfo'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr')), $fileName . '.xlsx');
        }

        return view('reportCrnt.eventMarkingState.index', compact('activeTrainingYearList', 'courseList'
                        , 'eventList', 'sortByList', 'assignedObsnInfo', 'gradeInfo'
                        , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'));
    }
    ...
    public function getCourse(Request $request) {
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + Course::where('training_year_id', $request->training_year_id)
                        ...
                        ->pluck('name', 'id')
                        ->toArray();
        $html = view('reportCrnt.eventMarkingState.getCourse', compact('courseList'))->render();
...
//            $request->sort = !empty($request->sort) ? $request->sort : 'position';
        ...
        if ($validator->fails()) {
            return redirect('eventMarkingStateReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('eventMarkingStateReportCrnt?generate=true&' . $url);
    }
