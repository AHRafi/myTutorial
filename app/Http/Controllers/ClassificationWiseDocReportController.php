...
class ClassificationWiseDocReportController extends Controller {
    ...
    public function index(Request $request) {
        ...
        if ($request->generate == 'true') {
            ...
            if (!empty($request->classification_id)) {
                $contentArr = $contentArr->where('content.content_classification_id', $request->classification_id);
            }
            ...
            $fileName = 'Classification_Wise_Document_Report';
            $fileName = Common::getFileFormatedName($fileName);
        }
        if ($request->view == 'print') {
            return view('referenceArchive.classificationWiseDocReport.print.index')->with(compact('request', 'contentClassificationList', 'contentModuleList', 'targetArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('referenceArchive.classificationWiseDocReport.print.index', compact('request', 'contentClassificationList', 'contentModuleList', 'targetArr'))
                    ->setPaper('a4', 'landscape')
                    ->setOptions(['defaultFont' => 'sans-serif']);

            return $pdf->download($fileName . '.pdf');
        } elseif ($request->view == 'excel') {
            return Excel::download(new ExcelExport('referenceArchive.classificationWiseDocReport.print.index', compact('request'
                                    , 'contentClassificationList', 'contentModuleList', 'targetArr'), 3), $fileName . '.xlsx');
        } else {

            return view('referenceArchive.classificationWiseDocReport.index', compact('request', 'contentClassificationList', 'contentModuleList', 'contentDetailsInfoArr', 'targetArr'));
        }
    }
...
//        print_r($request->all());
        $rules = $messages = [];
        $url = 'classification_id=' . $request->classification_id;

        $validator = Validator::make($request->all(), $rules, $messages);
        if ($validator->fails()) {
            return redirect('classificationWiseDocReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }

        return redirect('classificationWiseDocReport?generate=true&' . $url);
    }
