...
class EventResultCombinedReportController extends Controller {

    private $controller = 'EventResultCombinedReport';
...
//            exit;
        ...
        if ($request->view == 'print') {
            return view('report.eventResultCombined.print.index')->with(compact('activeTrainingYearList', 'courseList'
                                    , 'eventList', 'sortByList', 'assignedObsnInfo', 'gradeInfo'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('report.eventResultCombined.print.index', compact('activeTrainingYearList', 'courseList'
                                    , 'eventList', 'sortByList', 'assignedObsnInfo', 'gradeInfo'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'))
            ...
            return Excel::download(new ExcelExport('report.eventResultCombined.print.index', compact('activeTrainingYearList', 'courseList'
                                    , 'eventList', 'sortByList', 'assignedObsnInfo', 'gradeInfo'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr')), $fileName . '.xlsx');
        }

        return view('report.eventResultCombined.index', compact('activeTrainingYearList', 'courseList'
                        , 'eventList', 'sortByList', 'assignedObsnInfo', 'gradeInfo'
                        , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'));
    }
    ...
    public function getCourse(Request $request) {
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + Course::where('training_year_id', $request->training_year_id)
                        ...
                        ->pluck('name', 'id')
                        ->toArray();
        $html = view('report.eventResultCombined.getCourse', compact('courseList'))->render();
    ...
    public function getEvent(Request $request) {
        $eventList = ['0' => __('label.ALL_EVENT')] + TermToEvent::join('event', 'event.id', '=', 'term_to_event.event_id')
                        ...
                        ->pluck('event.event_code', 'event.id')
                        ->toArray();
        $html = view('report.eventResultCombined.getEvent', compact('eventList'))->render();
        return Response::json(['html' => $html]);
    }
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('eventResultCombinedReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('eventResultCombinedReport?generate=true&' . $url);
    }
