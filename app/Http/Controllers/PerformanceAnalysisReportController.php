...
class PerformanceAnalysisReportController extends Controller {

    private $controller = 'PerformanceAnalysisReport';
...
// comdt moderation wise mks & wt 
        ...
        if ($request->view == 'print') {
            return view('report.performanceAnalysis.print.index')->with(compact('activeTrainingYearList', 'courseList'
                                    , 'cmList', 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'request'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('report.performanceAnalysis.print.index', compact('activeTrainingYearList', 'courseList'
                                    , 'cmList', 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'request'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'))
            ...
            return Excel::download(new ExcelExport('report.performanceAnalysis.print.index', compact('activeTrainingYearList', 'courseList'
                                    , 'cmList', 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'request'
                                    , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr')), $fileName . '.xlsx');
        }

        return view('report.performanceAnalysis.index', compact('activeTrainingYearList', 'courseList'
                        , 'cmList', 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'request'
                        , 'eventMksWtArr', 'cmArr', 'rowSpanArr', 'achieveMksWtArr'));
    }
    ...
    public function getCourse(Request $request) {
        ...
        $html1 = view('report.performanceAnalysis.getCm', compact('cmList'))->render();
        $html = view('report.performanceAnalysis.getCourse', compact('courseList'))->render();
    ...
    public function getCm(Request $request) {
        ...
        $html = view('report.performanceAnalysis.getCm', compact('cmList'))->render();
...
//        echo '<pre>';            print_r($request->all()); exit;  
        ...
        if ($validator->fails()) {
            return redirect('performanceAnalysisReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('performanceAnalysisReport?generate=true&' . $url);
    }
