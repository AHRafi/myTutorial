...
class ArmsServiceWisePerformanceTrendReportController extends Controller {
...
// Get Assigned CI obsn wt
        ...
        return view('report.armsServiceWisePerformanceTrend.index', compact('activeTrainingYearList'
    ...
    public function getCourse(Request $request) {
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + Course::where('training_year_id', $request->training_year_id)
                        ...
                        ->pluck('name', 'id')
                        ->toArray();
        $html = view('report.armsServiceWisePerformanceTrend.getCourse', compact('courseList'))->render();
    ...
    public function getCourseWiseArmsService(Request $request) {
        ...
        $showArmsServiceView = view('report.armsServiceWisePerformanceTrend.getCourseWiseArmsService', compact('armsServiceList'))->render();
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('armsServiceWisePerformanceTrendReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('armsServiceWisePerformanceTrendReport?generate=true&' . $url);
    }
