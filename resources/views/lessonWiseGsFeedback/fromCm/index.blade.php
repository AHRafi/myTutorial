    ...
    <div class="col-md-12">
        ...
        <div class="portlet box green">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-line-chart"></i>@lang('label.LESSON_WISE_GS_FEEDBACK_FROM_CM')
                </div>
            </div>
            <div class="portlet-body">
                {!! Form::open([
                    'group' => 'form',
                    'url' => 'lessonWiseGsFeedbackFromCm/filter',
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $(document).on("change", "#trainingYearId", function() {
                ...
                $.ajax({
                    url: "{{ URL::to('lessonWiseGsFeedbackFromCm/getCourse') }}",
                    type: "POST",
                    dataType: "json",
                    headers: {
            ...
            $(document).on("change", "#courseId", function() {
                ...
                $.ajax({
                    url: "{{ URL::to('lessonWiseGsFeedbackFromCm/getGs') }}",
                    type: "POST",
                    dataType: "json",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            $(document).on("change", "#gsId", function() {
                ...
                $.ajax({
                    url: "{{ URL::to('lessonWiseGsFeedbackFromCm/getLesson') }}",
                    type: "POST",
                    dataType: "json",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
