...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        ...
        <div class="portlet-body">
            {!! Form::open(array('group' => 'form', 'url' => 'armsServiceWiseEventTrendReportCrnt/filter','class' => 'form-horizontal', 'id' => 'submitForm')) !!}
            <div class="row">
                <div class="col-md-2">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div id="armsServiceWiseEventTrendChart" style="width: 100%; height: 400px; margin: 0 auto;"></div>
                </div>
            </div>
...
//START::Get Course
    $(document).on("change", "#trainingYearId", function () {
        ...
        $.ajax({
            url: "{{ URL::to('armsServiceWiseEventTrendReportCrnt/getCourse')}}",
            type: "POST",
            dataType: "json",
            headers: {
...
//START::Get Course Wise Arms Service & Event
    $(document).on("change", "#courseId", function () {
        ...
        $.ajax({
            url: "{{ URL::to('armsServiceWiseEventTrendReportCrnt/getTerm')}}",
            type: "POST",
            dataType: "json",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    ...
    $(document).on("change", "#termId", function () {
        ...
        $.ajax({
            url: "{{ URL::to('armsServiceWiseEventTrendReportCrnt/getCourseWiseArmsServiceEvent')}}",
            type: "POST",
            dataType: "json",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    ...
    foreach ($selectedArmsServices as $armsServiceId => $armsServiceCode) {
        ...
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $max = !empty($armsSvcWiseMksArr[$armsServiceId][$eventId]['max']) ? $armsSvcWiseMksArr[$armsServiceId][$eventId]['max'] : 0;
                $min = !empty($armsSvcWiseMksArr[$armsServiceId][$eventId]['min']) ? $armsSvcWiseMksArr[$armsServiceId][$eventId]['min'] : 0;
                ?>
                    max = <?php echo $max; ?>;
                    min = <?php echo $min; ?>;
...
if (!empty($selectedArmsServices)) {
    foreach ($selectedArmsServices as $armsServiceId => $armsServiceCode) {
        ...
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $mks = !empty($armsSvcWiseMksArr[$armsServiceId][$eventId]['mks_percent']) ? $armsSvcWiseMksArr[$armsServiceId][$eventId]['mks_percent'] : 0;
                echo $mks . ',';
            }
        }
        ?>
...
//            forceNiceScale: true,
    ...
    var armsServiceWiseMksOptions2 = {
        chart: {
            height: 400,
            type: 'line',
            shadow: {
...
if (!empty($selectedArmsServices)) {
    foreach ($selectedArmsServices as $armsServiceId => $armsServiceCode) {
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $mks = !empty($armsSvcWiseMksArr[$armsServiceId][$eventId]['max']) ? $armsSvcWiseMksArr[$armsServiceId][$eventId]['max'] : 0;
                echo $mks . ',';
            }
        }
    }
...
if (!empty($selectedArmsServices)) {
    foreach ($selectedArmsServices as $armsServiceId => $armsServiceCode) {
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $mks = !empty($armsSvcWiseMksArr[$armsServiceId][$eventId]['min']) ? $armsSvcWiseMksArr[$armsServiceId][$eventId]['min'] : 0;
                echo $mks . ',';
            }
        }
    }
...
if (!empty($selectedArmsServices)) {
    foreach ($selectedArmsServices as $armsServiceId => $armsServiceCode) {
        echo "'$armsServiceCode',";
    }
}
?>
            ],
            title: {
                text: "@lang('label.ARMS_SERVICES')",
                style: {
                    color: undefined,
                    fontSize: '11px',
                    fontFamily: 'Helvetica, Arial, sans-serif',
...
if (!empty($selectedEvents)) {
    if (sizeof($selectedEvents) > 1) {
        ?>
            var armsServiceWiseMks = new ApexCharts(document.querySelector("#armsServiceWiseEventTrendChart"), armsServiceWiseMksOptions);
        <?php
    } else {
        ?>
            var armsServiceWiseMks = new ApexCharts(document.querySelector("#armsServiceWiseEventTrendChart"), armsServiceWiseMksOptions2);
        <?php
    }
    ?>
