...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-line-chart"></i>@lang('label.WING_WISE_EVENT_TREND')
            </div>
        </div>
        <div class="portlet-body">
            {!! Form::open(array('group' => 'form', 'url' => 'wingWiseEventTrendReportCrnt/filter','class' => 'form-horizontal', 'id' => 'submitForm')) !!}
            <div class="row">
                <div class="col-md-2">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <div class="col-md-8" id ="showWing">
                            {!! Form::select('wing_id[]', $wingList, $wingIds,  ['multiple'=>'multiple', 'class' => 'form-control', 'id' => 'wingId', 'data-width' => '100%']) !!}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div id="wingWiseEventTrendChart" style="width: 100%; height: 400px; margin: 0 auto;"></div>
                </div>
            </div>
...
//START:: Multiselect arms services
    var wingAllSelected = false;
    $('#wingId').multiselect({
        numberDisplayed: 0,
        includeSelectAllOption: true,
        buttonWidth: 'inherit',
        maxHeight: 250,
        nonSelectedText: "@lang('label.SELECT_WING_OPT')",
        enableCaseInsensitiveFiltering: true,
        onSelectAll: function () {
            wingAllSelected = true;
        },
        onChange: function () {
            wingAllSelected = false;
        }
    });
...
//START::Get Course
    $(document).on("change", "#trainingYearId", function () {
        ...
        $.ajax({
            url: "{{ URL::to('wingWiseEventTrendReportCrnt/getCourse')}}",
            type: "POST",
            dataType: "json",
            headers: {
...
//START::Get Course Wise Arms Service & Event
    $(document).on("change", "#courseId", function () {
        ...
        $.ajax({
            url: "{{ URL::to('wingWiseEventTrendReportCrnt/getTerm')}}",
            type: "POST",
            dataType: "json",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    ...
    $(document).on("change", "#termId", function () {
        ...
        $.ajax({
            url: "{{ URL::to('wingWiseEventTrendReportCrnt/getCourseWiseWingEvent')}}",
            type: "POST",
            dataType: "json",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
<?php
if (!empty($selectedWings)) {
    foreach ($selectedWings as $wingId => $wingCode) {
        ?>
            counter2 = 0;
            maxMinArr[counter1] = [];
        <?php
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $max = !empty($wingWiseMksArr[$wingId][$eventId]['max']) ? $wingWiseMksArr[$wingId][$eventId]['max'] : 0;
                $min = !empty($wingWiseMksArr[$wingId][$eventId]['min']) ? $wingWiseMksArr[$wingId][$eventId]['min'] : 0;
                ?>
                    max = <?php echo $max; ?>;
                    min = <?php echo $min; ?>;
...
if (!empty($selectedWings)) {
    foreach ($selectedWings as $wingId => $wingCode) {
        ...
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $mks = !empty($wingWiseMksArr[$wingId][$eventId]['mks_percent']) ? $wingWiseMksArr[$wingId][$eventId]['mks_percent'] : 0;
                echo $mks . ',';
            }
        }
        ?>
...
if (!empty($selectedWings)) {
    foreach ($selectedWings as $wingId => $wingCode) {
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $mks = !empty($wingWiseMksArr[$wingId][$eventId]['max']) ? $wingWiseMksArr[$wingId][$eventId]['max'] : 0;
                echo $mks . ',';
            }
        }
    }
...
if (!empty($selectedWings)) {
    foreach ($selectedWings as $wingId => $wingCode) {
        if (!empty($selectedEvents)) {
            foreach ($selectedEvents as $eventId => $eventCode) {
                $mks = !empty($wingWiseMksArr[$wingId][$eventId]['min']) ? $wingWiseMksArr[$wingId][$eventId]['min'] : 0;
                echo $mks . ',';
            }
        }
    }
...
if (!empty($selectedEvents)) {
    if (sizeof($selectedEvents) > 1) {
        ?>
            var wingWiseMks = new ApexCharts(document.querySelector("#wingWiseEventTrendChart"), wingWiseMksOptions);
        <?php
    } else {
        ?>
            var wingWiseMks = new ApexCharts(document.querySelector("#wingWiseEventTrendChart"), wingWiseMksOptions2);
        <?php
    }
    ?>
        wingWiseMks.render();
