...
<div class="col-md-12">
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-unlock"></i>@lang('label.UNLOCK_COMDT_OBSN_MARKING')
            </div>
        </div>
        <div class="portlet-body">
            @if(!empty($courseArr))
            {!! Form::open(array('group' => 'form', 'url' => 'unlockComdtObsnMarking/filter','class' => 'form-horizontal')) !!}
<!--            <div class="row">
                 Begin Filter
                <div class="col-md-12">
                    <div class="col-md-4">
    ...
    $(function () {
        $(document).on('click', '#unlockRequest', function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        url: "{{URL::to('unlockComdtObsnMarking/unlockRequest')}}",
                        type: "POST",
                        datatype: 'json',
                        cache: false,
                        headers: {
                        ...
                        success: function (res) {
                            toastr.success('@lang("label.COMDT_OBSN_MARKING_HAS_BEEN_UNLOCKED_SUCCESSFULLY")', res, options);
                            setTimeout(function () {
        ...
        $(document).on('click', '#denyRequest', function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{URL::to('unlockComdtObsnMarking/denyRequest')}}",
                        type: "POST",
                        datatype: 'json',
                        cache: false,
                        headers: {
                        ...
                        success: function (res) {
                            toastr.success('@lang("label.COMDT_OBSN_MARKING_HAS_BEEN_DENIED_TO_UNLOCK")', res, options);
                            setTimeout(function () {
                                document.location.reload(true);
                            }, 1000);
                            App.unblockUI();
