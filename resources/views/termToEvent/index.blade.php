...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-line-chart"></i>@lang('label.RELATE_TERM_TO_EVENT')
            </div>
        </div>
...
//
//            $.ajax({
//                url: "{{ URL::to('termToEvent/getTerm')}}",
//                type: "POST",
//                dataType: "json",
//                headers: {
...
//        });
        ...
        $(document).on("change", "#termId", function () {
            ...
            $.ajax({
                url: "{{ URL::to('termToEvent/getEvent')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on('click', '.button-submit', function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{URL::to('termToEvent/saveTermToEvent')}}",
                        type: "POST",
                        datatype: 'json',
                        cache: false,
                        contentType: false,
...
//                            $(document).trigger("change", "#termId");
        ...
        $(document).on('click', '#buttonDelete', function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    $.ajax({
                        url: "{{URL::to('termToEvent/deleteTermToEvent')}}",
                        type: "POST",
                        datatype: 'json',
                        cache: false,
                        contentType: false,
                        ...
                        success: function (res) {
                            toastr.success(res, '@lang("label.EVENT_DELETED_SUCCESSFULLY")', options);
        ...
        $(document).on("click", "#assignedEvent", function (e) {
            ...
            var termId = $("#termId").val();
            $.ajax({
                url: "{{ URL::to('termToEvent/getAssignedEvent')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
