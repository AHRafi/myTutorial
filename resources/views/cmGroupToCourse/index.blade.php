...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-line-chart"></i>@lang('label.RELATE_CM_GROUP_TO_COURSE')
            </div>
        </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div id="showCmGroup">
                        ...
                        <div class = "form-group">
                            <label class = "control-label col-md-4" for = "moduleId">@lang('label.CHOOSE_CM_GROUP') :<span class = "text-danger"> *</span></label>
                                ...
                                <div class="form-group form-md-line-input table-responsive max-height-300 webkit-scrollbar">
                                    <div class="col-md-10">
                                        <div class="md-checkbox-list">
                                            ...
                                            ?>
                                            <div class="md-checkbox">
                                                {!! Form::checkbox('cm_group_id['.$item->id.']',$item->id, false, ['id' => $item->id, 'data-id'=>$item->id,'class'=> 'md-check cm-group-to-course-check', $checked,$disabled]) !!}
                                                @if(!empty($disableCmGroup))
                                                @foreach($disableCmGroup as $key=>$value )
                                                {!! Form::hidden('cm_group_id['.$key.']', $key) !!}
                                                @endforeach
                                                @endif
                                                @if(!empty($disabled))
                                                {!! Form::hidden('cm_group_id['.$item->id.']', $item->id) !!}
                                                @endif
                                                <span class = "text-danger">{{ $errors->first('cm_group_id') }}</span>
                                                <label for="{{$item->id}}">
                                                    <span></span>
                                                    <span class="check tooltips" title="{{$title}}"></span>
                                                    <span class="box tooltips" title="Checked"></span>{{$item->name}}
                                                </label>
                                            </div>
                                            @endforeach
                                        </div>
                        ...
                        <div class = "form-actions">
                            <div class = "col-md-offset-4 col-md-8">
                                ...
                                <a href = "{{ URL::to('cmGroupToCourse') }}" class = "btn btn-circle btn-outline grey-salsa">@lang('label.CANCEL')</a>
                            </div>
                        </div>
                        @else
                        <div class="col-md-12">
                            <div class="alert alert-danger alert-dismissable">
                                <p><i class="fa fa-bell-o fa-fw"></i>@lang('label.NO_CM_GROUP_FOUND')</p>
                            </div>
                        </div>
                        @endif
                    </div>
...
//            }
//            $.ajax({
//                url: "{{ URL::to('cmGroupToCourse/getCmGroup')}}",
//                type: "POST",
//                dataType: "json",
//                headers: {
        ...
        swal({
            ...
            if (isConfirm) {
                $.ajax({
                    ...
                    data: form_data,
                    success: function (res) {
                        toastr.success(res, "@lang('label.CM_GROUP_HAS_BEEN_RELATED_TO_THIS_COURSE')", options);
