...
class DailyDocReportController extends Controller {
    ...
    public function index(Request $request) {
        ...
        if ($request->generate == 'true') {
            ...
            $date = !empty($request->date_of_upload) ? date("Y-m-d", strtotime($request->date_of_upload)) : '';
            if (!empty($date)) {
            ...
            $fileName = 'Daily_Document_Upload_Report';
            $fileName = Common::getFileFormatedName($fileName);
        }

        if ($request->view == 'print') {
            return view('referenceArchive.dailyDocReport.print.index')->with(compact('request', 'contentDetailsInfoArr', 'contentModuleList', 'targetArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('referenceArchive.dailyDocReport.print.index', compact('request', 'contentDetailsInfoArr', 'contentModuleList', 'targetArr'))
                    ->setPaper('a4', 'landscape')
                    ->setOptions(['defaultFont' => 'sans-serif']);

            return $pdf->download($fileName . '.pdf');
        } elseif ($request->view == 'excel') {
            return Excel::download(new ExcelExport('referenceArchive.dailyDocReport.print.index', compact('request', 'contentModuleList', 'contentDetailsInfoArr'
                                    , 'targetArr'), 3), $fileName . '.xlsx');
        } else {

            return view('referenceArchive.dailyDocReport.index', compact('request', 'contentModuleList', 'contentDetailsInfoArr', 'targetArr'));
        }
    }
