...
class SubSubEventController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $nameArr = SubSubEvent::select('event_code')->orderBy('event_code', 'asc')->get();

        //passing param for custom function
        $qpArr = $request->all();

        $targetArr = SubSubEvent::select('sub_sub_event.id', 'sub_sub_event.event_code', 'sub_sub_event.event_detail', 'sub_sub_event.status'
                , 'sub_sub_event.order', 'sub_sub_event.hidden')
                ->orderBy('sub_sub_event.status', 'asc')
        ...
        {
            $targetArr = $targetArr->where('sub_sub_event.hidden', '0');
        }
        ...
        if ($request->download == 'pdf') {
            ...
            return view('subSubEvent.index')->with(compact('targetArr', 'qpArr', 'nameArr', 'hiddenOptionList'));
        }
    }
...
//Check Dependency before deletion
        $dependencyArr = [
            'EventToSubSubEvent' => 'sub_sub_event_id'
...
//            , 'RctState' => 'syndicate_id', 'RecruitToSyndicate' => 'syndicate_id'
        ...
        foreach ($dependencyArr as $model => $key) {
            ...
            if (!empty($dependentData)) {
                ...
                return redirect('subSubEvent?page=' . $pageNumber);
            }
        }
    ...
    public function hideShow(Request $request) {
        ...
        $target = SubSubEvent::find($request->id);
        if (empty($target)) {
