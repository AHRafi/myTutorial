...
//                        ->where('status', '1')->orderBy('id', 'desc')->pluck('name', 'id')->toArray();
        ...
        return view('termToSubSubSubEvent.index')->with(compact('activeTrainingYearInfo', 'activeCourse', 'termList'
...
//    }
    ...
    public function getEvent(Request $request) {
        ...
        $eventList = ['0' => __('label.SELECT_EVENT_OPT')] + TermToEvent::join('event', 'event.id', '=', 'term_to_event.event_id')
                        ...
                        ->join('event_to_sub_sub_sub_event', function($join) {
                            $join->on('event_to_sub_sub_sub_event.event_id', '=', 'term_to_event.event_id');
                        })
    ...
    public function getSubEvent(Request $request) {
        ...
        $subEventList = ['0' => __('label.SELECT_SUB_EVENT_OPT')] + TermToSubEvent::join('sub_event', 'sub_event.id', '=', 'term_to_sub_event.sub_event_id')
                        ...
                        ->join('event_to_sub_sub_sub_event', function($join) {
                            $join->on('event_to_sub_sub_sub_event.event_id', '=', 'term_to_sub_event.event_id');
                            $join->on('event_to_sub_sub_sub_event.sub_event_id', '=', 'term_to_sub_event.sub_event_id');
                        })
    ...
    public function getSubSubEvent(Request $request) {
        ...
        $subSubEventList = ['0' => __('label.SELECT_SUB_SUB_EVENT_OPT')] + TermToSubSubEvent::join('sub_sub_event', 'sub_sub_event.id', '=', 'term_to_sub_sub_event.sub_sub_event_id')
                        ...
                        ->join('event_to_sub_sub_sub_event', function($join) {
                            $join->on('event_to_sub_sub_sub_event.event_id', '=', 'term_to_sub_sub_event.event_id');
                            $join->on('event_to_sub_sub_sub_event.sub_event_id', '=', 'term_to_sub_sub_event.sub_event_id');
                            $join->on('event_to_sub_sub_sub_event.sub_sub_event_id', '=', 'term_to_sub_sub_event.sub_sub_event_id');
                        })
...
//        echo '<pre>';        print_r($subSubEventList); exit;
    ...
    public function getSubSubSubEvent(Request $request) {
        ...
        $html = view('termToSubSubSubEvent.getSubSubSubEvent', compact('targetArr', 'prevDataArr', 'termList', 'prevDataList'
                        , 'prevTermToSubSubSubEventList', 'request', 'prevSaveTermToSubSubSubEventList'
                        , 'eventAssessmentMarkingData'))->render();
        return response()->json(['html' => $html]);
    }
    ...
    public function deleteTermToSubSubSubEvent(Request $request) {
        // Delete previous record for this course_id
        $termToSubSubSubEvent = TermToSubSubSubEvent::where('course_id', $request->course_id)
                ->where('term_id', $request->term_id)
                ->where('event_id', $request->event_id)
                ->where('sub_event_id', $request->sub_event_id)
                ->where('sub_sub_event_id', $request->sub_sub_event_id);

        if ($termToSubSubSubEvent->delete()) {
            return Response::json(['success' => true], 200);
        } else {
            return Response::json(array('success' => false, 'message' => 'Sub Sub Sub Event could not be deleted'), 401);
        }
    }
