...
class WingWisePerformanceTrendReportCrntController extends Controller {
    ...
    public function index(Request $request) {
        ...
        if (empty($activeTrainingYearList)) {
            $void['header'] = __('label.WING_WISE_PERFORMANCE_TREND');
        ...
        $eventMksWtArr = $eventWiseMksArr = $wingWiseMksArr = $gradeList = $wingIds = $selectedWings = $wingTotalCm = $wingWiseMksPer = [];
        ...
        return view('reportCrnt.wingWisePerformanceTrend.index', compact('activeTrainingYearList'
    ...
    public function getCourse(Request $request) {
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + Course::where('training_year_id', $request->training_year_id)
                        ...
                        ->pluck('name', 'id')
                        ->toArray();
        $html = view('reportCrnt.wingWisePerformanceTrend.getCourse', compact('courseList'))->render();
    ...
    public function getCourseWiseWing(Request $request) {
        ...
        $showWingView = view('reportCrnt.wingWisePerformanceTrend.getCourseWiseWing', compact('wingList'))->render();
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('wingWisePerformanceTrendReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('wingWisePerformanceTrendReportCrnt?generate=true&' . $url);
    }
