...
class CourseProgressiveResultReportController extends Controller {

    private $controller = 'CourseProgressiveResultReport';
...
//            get comdt observation
        ...
        if ($request->view == 'print') {
            return view('report.courseProgressiveResult.print.index')->with(compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                                    , 'eventMksWtArr', 'cmArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('report.courseProgressiveResult.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                                    , 'eventMksWtArr', 'cmArr'))
            ...
            return Excel::download(new ExcelExport('report.courseProgressiveResult.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                                    , 'eventMksWtArr', 'cmArr')), $fileName . '.xlsx');
        }

        return view('report.courseProgressiveResult.index', compact('activeTrainingYearList', 'courseList', 'termList'
                        , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                        , 'eventMksWtArr', 'cmArr'));
    }
    ...
    public function getCourse(Request $request) {
        ...
        $html = view('report.courseProgressiveResult.getCourse', compact('courseList'))->render();
    ...
    public function getTerm(Request $request) {
        ...
        $html = view('report.courseProgressiveResult.getTerm', compact('termList'))->render();
...
//        echo '<pre>';            print_r($request->all()); exit;  
        ...
        if ($validator->fails()) {
            return redirect('courseProgressiveResultReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('courseProgressiveResultReport?generate=true&' . $url);
    }
