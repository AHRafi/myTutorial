...
class DsRemarksReportController extends Controller {

    private $controller = 'DsRemarksReport';
...
//            exit;
        ...
        if ($request->view == 'print') {
            return view('report.dsRemarks.print.index')->with(compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'eventList', 'cmList', 'dsRemarksArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('report.dsRemarks.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'eventList', 'cmList', 'dsRemarksArr'))
        ...
        return view('report.dsRemarks.index', compact('activeTrainingYearList', 'courseList', 'eventList', 'termList'
                        , 'cmList', 'dsRemarksArr'));
    }
    ...
    public function getCourse(Request $request) {
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + Course::where('training_year_id', $request->training_year_id)
                        ...
                        ->pluck('name', 'id')
                        ->toArray();
        $html = view('report.dsRemarks.getCourse', compact('courseList'))->render();
    ...
    public function getTerm(Request $request) {
        ...
        $html = view('report.dsRemarks.getTerm', compact('termList'))->render();
        $html2 = view('report.dsRemarks.getCm', compact('cmList'))->render();
    ...
    public function getEvent(Request $request) {
        ...
        $html = view('report.dsRemarks.getEvent', compact('eventList'))->render();
        return Response::json(['html' => $html]);
    }
...
//            'cm_id.not_in' => __('label.THE_CM_FIELD_IS_REQUIRED'),
        ...
        if ($validator->fails()) {
            return redirect('dsRemarksReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('dsRemarksReport?generate=true&' . $url);
    }
