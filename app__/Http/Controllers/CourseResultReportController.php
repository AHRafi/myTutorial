...
class CourseResultReportController extends Controller {

    private $controller = 'CourseResultReport';
...
//            exit;
        ...
        if ($request->view == 'print') {
            return view('report.courseResult.print.index')->with(compact('activeTrainingYearList', 'courseList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                                    , 'eventMksWtArr', 'cmArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('report.courseResult.print.index', compact('activeTrainingYearList', 'courseList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                                    , 'eventMksWtArr', 'cmArr'))
            ...
            return Excel::download(new ExcelExport('report.courseResult.print.index', compact('activeTrainingYearList', 'courseList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                                    , 'eventMksWtArr', 'cmArr')), $fileName . '.xlsx');
        }

        return view('report.courseResult.index', compact('activeTrainingYearList', 'courseList'
                        , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                        , 'eventMksWtArr', 'cmArr'));
    }
    ...
    public function getCourse(Request $request) {
        ...
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + $courseList;
        $html = view('report.courseResult.getCourse', compact('courseList'))->render();
...
//        echo '<pre>';            print_r($request->all()); exit;  
        ...
        if ($validator->fails()) {
            return redirect('courseResultReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('courseResultReport?generate=true&' . $url);
    }
