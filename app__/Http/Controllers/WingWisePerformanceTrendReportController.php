...
class WingWisePerformanceTrendReportController extends Controller {

    private $controller = 'ArmsServiceWisePerformanceTrendReport';
    ...
    public function index(Request $request) {
        ...
        $eventMksWtArr = $eventWiseMksArr = $wingWiseMksArr = $wingWiseMksPer = $gradeList = $wingIds = $selectedWings = [];
        ...
        if ($request->generate == 'true') {
            ...
            if (!empty($cmArr)) {
                foreach ($cmArr as $cmId => $wingid) {
                    $wingTotalCm[$wingid] = !empty($wingTotalCm[$wingid]) ? $wingTotalCm[$wingid] : 0;
                    $wingTotalCm[$wingid] += 1;
                }
            }
                ...
                foreach ($cmWiseMksArr as $cmId => $wtInfo) {
                    ...
                    if (!empty($gradeArr)) {
                        foreach ($gradeArr as $letter => $gradeRange) {
                            $wingWiseMksArr[$gradeRange['id']][$cmArr[$cmId]] = !empty($wingWiseMksArr[$gradeRange['id']][$cmArr[$cmId]]) ? $wingWiseMksArr[$gradeRange['id']][$cmArr[$cmId]] : 0;
                            
                            if ($totalPercentage == 100) {
                                $wingWiseMksArr[$gradeRange['id']][$cmArr[$cmId]] += 1;
            ...
            $maxCmArr = [];
            if (!empty($wingWiseMksPer)) {
                foreach ($wingWiseMksPer as $gradeId => $gradeInfo) {
                    $maxCmArr[$gradeId] = max($gradeInfo);
                    foreach ($gradeInfo as $wingId => $number) {
                        $number = !empty($number) ? $number : 0;
                        $numberOfCm = !empty($wingTotalCm[$wingId]) ? $wingTotalCm[$wingId] : 0;
        ...
        return view('report.wingWisePerformanceTrend.index', compact('activeTrainingYearList'
                        , 'courseList', 'selectedWings', 'cmArr', 'maxCm'
                        , 'assignedObsnInfo', 'gradeInfo', 'request'
                        , 'wingList', 'wingIds', 'gradeList', 'wingWiseMksArr', 'wingWiseMksPer'));
    }
    ...
    public function getCourse(Request $request) {
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + Course::where('training_year_id', $request->training_year_id)
                        ...
                        ->pluck('name', 'id')
                        ->toArray();
        $html = view('report.wingWisePerformanceTrend.getCourse', compact('courseList'))->render();
        return Response::json(['html' => $html]);
    }

    public function getCourseWiseWing(Request $request) {
        $wingList = CmBasicProfile::join('wing', 'wing.id', '=', 'cm_basic_profile.wing_id')
                ->where('cm_basic_profile.course_id', $request->course_id)
                ->where('wing.status', '1')
                ->orderBy('wing.order', 'asc')
        ...
        $showWingView = view('report.wingWisePerformanceTrend.getCourseWiseWing', compact('wingList'))->render();
        return Response::json(['showWingView' => $showWingView]);
    }
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('wingWisePerformanceTrendReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('wingWisePerformanceTrendReport?generate=true&' . $url);
    }
