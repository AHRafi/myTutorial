...
class CmWiseEventTrendReportCrntController extends Controller {

    private $controller = 'CmWiseEventTrendReportCrnt';
    ...
    public function index(Request $request) {
        ...
        if (empty($activeTrainingYearList)) {
            $void['header'] = __('label.CM_WISE_EVENT_TREND');
...
//            exit;
        ...
        return view('reportCrnt.cmWiseEventTrend.index', compact('activeTrainingYearList', 'courseList', 'termList'
                        , 'cmArr', 'eventList', 'cmIds', 'cmWisePercentageArr', 'selectedCms', 'wingList'));
    }
    ...
    public function getCm(Request $request) {
        ...
        $cmArr = $cmArr->where('cm_basic_profile.status', '1')
                ->pluck('cm_name', 'cm_basic_profile.id')
                ->toArray();
        $html = view('reportCrnt.cmWiseEventTrend.getSvcWiseCm', compact('cmArr'))->render();
    ...
    public function getCourse(Request $request) {
        ...
        $html2 = view('reportCrnt.cmWiseEventTrend.getCourseWiseCm', compact('cmArr'))->render();
        $showEventView = view('reportCrnt.cmWiseEventTrend.getCourseWiseEvent', compact('eventList'))->render();
        $html = view('reportCrnt.cmWiseEventTrend.getCourse', compact('courseList'))->render();
        $html1 = view('reportCrnt.cmWiseEventTrend.getTerm', compact('termList'))->render();
    ...
    public function getTerm(Request $request) {
        ...
        $cmArr = CmBasicProfile::leftJoin('rank', 'rank.id', 'cm_basic_profile.rank_id')
                ...
                ->toArray();
        $eventList = [];
        $html = view('reportCrnt.cmWiseEventTrend.getCourseWiseCm', compact('cmArr'))->render();
        $html1 = view('reportCrnt.cmWiseEventTrend.getTerm', compact('termList'))->render();
    ...
    public function filter(Request $request) {
        ...
        $url = 'training_year_id=' . $request->training_year_id . '&course_id=' . $request->course_id
                . '&term_id=' . $request->term_id . '&wing_id=' . $request->wing_id . '&cm_id=' . $cmIds
                . '&event_id=' . $request->event_id
                . '&range_start=' . $request->range_start . '&range_end=' . $request->range_end;

        $validator = Validator::make($request->all(), $rules, $messages);
        if ($validator->fails()) {
            return redirect('cmWiseEventTrendReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('cmWiseEventTrendReportCrnt?generate=true&' . $url);
    }
