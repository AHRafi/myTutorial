...
class CommissioningCourseWiseEventTrendReportCrntController extends Controller {

    private $controller = 'CommissioningCourseWiseEventTrendReportCrnt';
    ...
    public function index(Request $request) {
        ...
        if (empty($activeTrainingYearList)) {
            $void['header'] = __('label.COMMISSIONING_COURSE_WISE_EVENT_TREND');
        ...
        $eventMksWtArr = $eventWiseMksArr = $commissioningCourseWiseMksArr = $cmMksMinMaxArr = [];
        $commissioningCourseIds = $eventIds = $selectedEvents = $selectedCommissioningCourses = [];
        if ($request->generate == 'true') {
            ...
            $selectedCommissioningCourses = CmBasicProfile::join('commissioning_course', 'commissioning_course.id', '=', 'cm_basic_profile.commissioning_course_id')
                            ...
                            ->where('commissioning_course.status', '1')
                            ->orderBy('commissioning_course.commissioning_date', 'asc')
                            ->pluck('commissioning_course.short_info', 'commissioning_course.id')->toArray();
            ...
            $cmWiseCommissioningCourseArr = [];
            if (!empty($cmArr)) {
                foreach ($cmArr as $cmId => $commissioningCourseId) {
                    $cmWiseCommissioningCourseArr[$commissioningCourseId][$cmId] = $cmId;
                }
            }
            ...
            if (!empty($cmWiseMksArr)) {
                foreach ($cmWiseMksArr as $cmId => $eventInfo) {
                    foreach ($eventInfo as $eventId => $mksInfo) {
                        ...
                        $commissioningCourseWiseMksArr[$cmArr[$cmId]][$eventId]['total_mks_percent'] = !empty($commissioningCourseWiseMksArr[$cmArr[$cmId]][$eventId]['total_mks_percent']) ? $commissioningCourseWiseMksArr[$cmArr[$cmId]][$eventId]['total_mks_percent'] : 0;
                        $commissioningCourseWiseMksArr[$cmArr[$cmId]][$eventId]['total_mks_percent'] += $eventPercentage;
                    }
                }
            }
            ...
            if (!empty($commissioningCourseWiseMksArr)) {
                foreach ($commissioningCourseWiseMksArr as $commissioningCourseId => $eventInfo) {
                    foreach ($eventInfo as $eventId => $mksInfo) {
                        $eventMks = !empty($mksInfo['total_mks_percent']) ? $mksInfo['total_mks_percent'] : 0;
                        $commissioningCourseWiseMksArr[$commissioningCourseId][$eventId]['mks_percent'] = ($eventMks) / (!empty($cmWiseCommissioningCourseArr[$commissioningCourseId]) ? sizeof($cmWiseCommissioningCourseArr[$commissioningCourseId]) : 1);

                        //commissioning_course wise max/min 
                        $commissioningCourseWiseMksArr[$commissioningCourseId][$eventId]['max'] = !empty($cmMksMinMaxArr[$eventId][$commissioningCourseId]) ? max($cmMksMinMaxArr[$eventId][$commissioningCourseId]) : 0;
                        $commissioningCourseWiseMksArr[$commissioningCourseId][$eventId]['min'] = !empty($cmMksMinMaxArr[$eventId][$commissioningCourseId]) ? min($cmMksMinMaxArr[$eventId][$commissioningCourseId]) : 0;
                        $commissioningCourseWiseMksArr['grand_max_min']['max'] = !empty($cmMksMinMaxArr['grand_max_min']) ? max($cmMksMinMaxArr['grand_max_min']) + (max($cmMksMinMaxArr['grand_max_min']) + 5 < 100 ? 5 : 0) : 0;
                        $commissioningCourseWiseMksArr['grand_max_min']['min'] = !empty($cmMksMinMaxArr['grand_max_min']) ? min($cmMksMinMaxArr['grand_max_min']) - (min($cmMksMinMaxArr['grand_max_min']) - 5 > 0 ? 5 : 0) : 0;
                    }
                }
            }
        }
        
        return view('reportCrnt.commissioningCourseWiseEventTrend.index', compact('activeTrainingYearList', 'courseList'
                        , 'commissioningCourseList', 'eventList', 'commissioningCourseIds', 'eventIds', 'termList'
                        , 'commissioningCourseWiseMksArr', 'selectedCommissioningCourses', 'selectedEvents'));
    }
    ...
    public function getCourse(Request $request) {
        ...
        $commissioningCourseList = [];

        $eventList = [];

        $html2 = view('reportCrnt.commissioningCourseWiseEventTrend.getCourseWiseCommissioningCourse', compact('commissioningCourseList'))->render();
        $showEventView = view('reportCrnt.commissioningCourseWiseEventTrend.getCourseWiseEvent', compact('eventList'))->render();
        $html = view('reportCrnt.commissioningCourseWiseEventTrend.getCourse', compact('courseList'))->render();
        $html1 = view('reportCrnt.commissioningCourseWiseEventTrend.getTerm', compact('termList'))->render();
    ...
    public function getTerm(Request $request) {
        ...
        $eventList = [];
        $html1 = view('reportCrnt.commissioningCourseWiseEventTrend.getTerm', compact('termList'))->render();
        $html = view('reportCrnt.commissioningCourseWiseEventTrend.getCourseWiseCommissioningCourse', compact('commissioningCourseList'))->render();
        $showEventView = view('reportCrnt.commissioningCourseWiseEventTrend.getCourseWiseEvent', compact('eventList'))->render();

        return response()->json(['html' => $html, 'html1' => $html1, 'showEventView' => $showEventView]);
    }
    
    public function getCourseWiseCommissioningCourseEvent(Request $request) {
    ...
    public function filter(Request $request) {
        ...
        $url = 'training_year_id=' . $request->training_year_id . '&course_id=' . $request->course_id
                . '&term_id=' . $request->term_id. '&commissioning_course_id=' . $commissioningCourseIds
                . '&event_id=' . $eventIds
                . '&range_start=' . $request->range_start . '&range_end=' . $request->range_end;

        $validator = Validator::make($request->all(), $rules, $messages);
        if ($validator->fails()) {
            return redirect('commissioningCourseWiseEventTrendReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('commissioningCourseWiseEventTrendReportCrnt?generate=true&' . $url);
    }
