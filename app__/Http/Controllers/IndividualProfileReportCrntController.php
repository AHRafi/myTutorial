...
class IndividualProfileReportCrntController extends Controller {
...
//        if (in_array(Auth::user()->group_id, [4]) && $deligatedDs != Auth::user()->id) {
//            $cmList = $cmList->whereIn('cm_basic_profile.id', $dsCmArr);
//        }
        ...
        $onTheFlyReportData = DeligateReportsToDs::where('course_id', $courseList->id)
                ->select('report', 'ds_id')
                ->first();
        $reportArr = !empty($onTheFlyReportData->report) ? explode(",", $onTheFlyReportData->report) : [];
        $reportDsArr = !empty($onTheFlyReportData->ds_id) ? explode(",", $onTheFlyReportData->ds_id) : [];
        $dsCanAccess = !empty($reportDsArr) && ($reportDsArr == '0' || in_array(Auth::user()->id, $reportDsArr)) ? 1 : 0;
		$ontheFlyReport = !empty($reportArr) && in_array(5, $reportArr) && !empty($dsCanAccess) ? 1 : 0;
                            ...
                            foreach ($subEvInfo as $subEventId => $subSubEvInfo) {
                                foreach ($subSubEvInfo as $subSubEventId => $subSubSubEvInfo) {
                                    foreach ($subSubSubEvInfo as $subSubSubEventId => $info) {
                                        ...
                                        $achievedMksWtArr['term_agg_percentage'] = 0;
                                        if (!empty($achievedMksWtArr['term_agg_total_assigned_wt'])) {
                                            $achievedMksWtArr['term_agg_percentage'] = ($achievedMksWtArr['term_agg_total_wt'] * 100) / ($achievedMksWtArr['term_agg_total_assigned_wt']);
                                        }
...
//                echo '<pre>';                print_r($cmArr);exit;
                ...
                $eventMksWtArr['final_assigned_wt'] = (!empty($achievedMksWtArr['term_agg_total_assigned_wt']) ? $achievedMksWtArr['term_agg_total_assigned_wt'] : 0);
                ...
                $comdtObsnInfo = ComdtObsnMarking::select('comdt_obsn', 'percentage')->where('course_id', $request->course_id)
                        ...
                        ->first();
                if (!empty($ciObsnInfo)) {
                    $eventMksWtArr['final_assigned_wt'] += (!empty($eventMksWtArr['ci_obsn_wt']) ? $eventMksWtArr['ci_obsn_wt'] : 0);
                    $achievedMksWtArr['ci_obsn'] = !empty($ciObsnInfo->ci_obsn) ? $ciObsnInfo->ci_obsn : 0;
                }
                if (!empty($comdtObsnInfo)) {
                    $eventMksWtArr['final_assigned_wt'] += (!empty($eventMksWtArr['comdt_obsn_wt']) ? $eventMksWtArr['comdt_obsn_wt'] : 0);
                ...
                $achievedMksWtArr['final_percent'] = 0;
                if (!empty($eventMksWtArr['final_assigned_wt'])) {
                    $achievedMksWtArr['final_percent'] = ($achievedMksWtArr['final_wt'] * 100) / $eventMksWtArr['final_assigned_wt'];
                }
                ...
                $cmArr = Common::getIndividualPosition($courseId, $cmId, $ontheFlyReport);
//                echo '<pre>';
//                print_r($cmArr);
//                exit;
                //End:: Get Position
                ...
                if ($request->view == 'print') {
                    return view('reportCrnt.individualProfile.print.profile')->with(compact('cmInfoData', 'cmArr', 'religionList', 'appointmentList', 'allAppointmentList', 'armsServiceList'
                    ...
                    $pdf = PDF::loadView('reportCrnt.individualProfile.print.profile', compact('cmInfoData', 'cmArr', 'religionList', 'appointmentList', 'allAppointmentList', 'armsServiceList'
                                            , 'unitList', 'maritalStatusList', 'countriesVisitedList', 'othersInfoData', 'addressInfo'
                                            , 'divisionList', 'districtList', 'thanaList', 'civilEducationInfoData', 'serviceRecordInfoData'
                                            , 'defenceRelativeInfoData', 'courseList', 'organizationList', 'passportInfoData'
                                            , 'presentAddressInfo', 'presentDistrictList', 'presentThanaList', 'milCourseList'
                ...
                return view('reportCrnt.individualProfile.profile')->with(compact('cmInfoData', 'cmArr', 'religionList', 'appointmentList', 'allAppointmentList', 'armsServiceList'
                                        , 'unitList', 'maritalStatusList', 'countriesVisitedList', 'othersInfoData', 'addressInfo'
                                        , 'divisionList', 'districtList', 'thanaList', 'civilEducationInfoData', 'serviceRecordInfoData'
                                        , 'defenceRelativeInfoData', 'courseList', 'organizationList', 'passportInfoData'
                                        , 'presentAddressInfo', 'presentDistrictList', 'presentThanaList', 'milCourseList'
...
//                if (in_array(Auth::user()->group_id, [4]) && $deligatedDs != Auth::user()->id) {
//                    $targetArr = $targetArr->whereIn('cm_basic_profile.id', $dsCmArr);
//                }
                $targetArr = $targetArr->where('cm_basic_profile.status', '1')
                        ->get();
                ...
                if ($request->view == 'print') {
                    return view('reportCrnt.individualProfile.print.index')->with(compact('request', 'activeTrainingYearList', 'courseList', 'termList'
                                            , 'targetArr', 'qpArr', 'cmList', 'spouseProfession'));
                } elseif ($request->view == 'pdf') {
                    $pdf = PDF::loadView('reportCrnt.individualProfile.print.index', compact('request', 'activeTrainingYearList', 'courseList', 'termList'
                    ...
                    return Excel::download(new ExcelExport('reportCrnt.individualProfile.print.index', compact('request', 'activeTrainingYearList', 'courseList', 'termList'
                                            , 'targetArr', 'qpArr', 'cmList')), $fileName . '.xlsx');
                }


                return view('reportCrnt.individualProfile.index', compact('request', 'activeTrainingYearList', 'courseList', 'termList'
                                , 'targetArr', 'qpArr', 'cmList', 'spouseProfession'));
            }
        }


        return view('reportCrnt.individualProfile.index')->with(compact('request', 'activeTrainingYearList', 'courseList', 'qpArr', 'cmList', 'spouseProfession')
        );
    }
    ...
    public function getCourse(Request $request) {
        ...
        $html = view('reportCrnt.individualProfile.getCourse', compact('courseList'))->render();
    ...
    public function getCm(Request $request) {
        ...
        $html = view('reportCrnt.individualProfile.getCm', compact('cmList'))->render();
        return Response::json(['html' => $html]);
    }
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('individualProfileReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('individualProfileReportCrnt?generate=true&' . $url);
    }

    public function profile(Request $request, $id) {
        $url = 'generate=' . $request->generate . '&training_year_id=' . $request->training_year_id . '&course_id=' . $request->course_id . '&cm_id=' . $id;
        return redirect('individualProfileReportCrnt?' . $url);
    }
