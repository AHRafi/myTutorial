...
class CourseProgressiveResultReportCrntController extends Controller {

    private $controller = 'CourseProgressiveResultReportCrnt';
...
//get only active training year
        ...
        if (empty($activeTrainingYearList)) {
            $void['header'] = __('label.COURSE_PROGRESSIVE_RESULT');
        ...
        $termList = $termList->orderBy('term.order', 'asc')
                ...
                ->toArray();
        $termList = ['0' => __('label.ALL_TERMS')] + $termList;
        ...
        if ($request->generate == 'true') {
            ...
            $fileName = 'Course_Progressive_Result' . $tyName . $courseName . $termName;
...
//            echo '<pre>';
//            print_r($eventMksWtArr['total_wt']);
            ...
            if (empty($request->sort) || $request->sort == 'position') {
                if (!empty($cmArr)) {
                    usort($cmArr, function ($item1, $item2) {
                        $positioning = !empty($request->term_id) ? 'term_agg_percentage' : 'final_percentage';
...
//            exit;
        ...
        if ($request->view == 'print') {
            return view('reportCrnt.courseProgressiveResult.print.index')->with(compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                                    , 'eventMksWtArr', 'cmArr'));
        } elseif ($request->view == 'pdf') {
            $pdf = PDF::loadView('reportCrnt.courseProgressiveResult.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                                    , 'eventMksWtArr', 'cmArr'))
            ...
            return Excel::download(new ExcelExport('reportCrnt.courseProgressiveResult.print.index', compact('activeTrainingYearList', 'courseList', 'termList'
                                    , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                                    , 'eventMksWtArr', 'cmArr')), $fileName . '.xlsx');
        }

        return view('reportCrnt.courseProgressiveResult.index', compact('activeTrainingYearList', 'courseList', 'termList'
                        , 'sortByList', 'assignedObsnInfo', 'gradeInfo', 'termDataArr', 'request'
                        , 'eventMksWtArr', 'cmArr'));
    }
    ...
    public function getCourse(Request $request) {
        ...
        $html = view('reportCrnt.courseProgressiveResult.getCourse', compact('courseList'))->render();
    ...
    public function getTerm(Request $request) {
        ...
        $html = view('reportCrnt.courseProgressiveResult.getTerm', compact('termList'))->render();
...
//        echo '<pre>';            print_r($request->all()); exit;  
        ...
        if ($validator->fails()) {
            return redirect('courseProgressiveResultReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('courseProgressiveResultReportCrnt?generate=true&' . $url);
    }
