...
class CmGroupWiseEventTrendReportCrntController extends Controller {

    private $controller = 'CmGroupWiseEventTrendReportCrnt';
    ...
    public function index(Request $request) {
        ...
        if (empty($activeTrainingYearList)) {
            $void['header'] = __('label.CM_GROUP_WISE_EVENT_TREND');
        ...
        $cmGroupList = CmGroupMemberTemplate::join('cm_group', 'cm_group.id', '=', 'cm_group_member_template.cm_group_id')
                        ->where('cm_group_member_template.course_id', $courseList->id)->where('cm_group.status', '1')
        ...
        return view('reportCrnt.cmGroupWiseEventTrend.index', compact('activeTrainingYearList', 'courseList'
    ...
    public function getCourse(Request $request) {
        ...
        $html2 = view('reportCrnt.cmGroupWiseEventTrend.getCourseWiseCmGroup', compact('cmGroupList'))->render();
        $showEventView = view('reportCrnt.cmGroupWiseEventTrend.getCourseWiseEvent', compact('eventList'))->render();
        $html = view('reportCrnt.cmGroupWiseEventTrend.getCourse', compact('courseList'))->render();
        $html1 = view('reportCrnt.cmGroupWiseEventTrend.getTerm', compact('termList'))->render();
    ...
    public function getTerm(Request $request) {
        ...
        $eventList = [];
        $html1 = view('reportCrnt.cmGroupWiseEventTrend.getTerm', compact('termList'))->render();
        $html = view('reportCrnt.cmGroupWiseEventTrend.getCourseWiseCmGroup', compact('cmGroupList'))->render();
        $showEventView = view('reportCrnt.cmGroupWiseEventTrend.getCourseWiseEvent', compact('eventList'))->render();
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('cmGroupWiseEventTrendReportCrnt?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('cmGroupWiseEventTrendReportCrnt?generate=true&' . $url);
    }
