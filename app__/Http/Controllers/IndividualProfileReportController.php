...
class IndividualProfileReportController extends Controller {
        ...
        if ($request->generate == 'true') {
            ...
            if (!empty($request->cm_id)) {
                ...
                $eventWiseMksWtInfo = EventAssessmentMarking::join('event_assessment_marking_lock', function($join) {
                        ...
                        ->whereIn('event_assessment_marking.term_id', $termIdList)
...
//sub sub sub event info
                $subSubSubEventInfo = TermToSubSubSubEvent::join('sub_sub_sub_event', 'sub_sub_sub_event.id', '=', 'term_to_sub_sub_sub_event.sub_sub_sub_event_id')
                        ...
                        ->select('sub_sub_sub_event.event_code as sub_sub_sub_event_code', 'sub_sub_sub_event.id as sub_sub_sub_event_id', 'sub_sub_sub_event_mks_wt.mks_limit', 'sub_sub_sub_event_mks_wt.highest_mks_limit'
                                ...
                                , 'term_to_sub_sub_sub_event.term_id'
                                , 'event_to_sub_event.avg_marking')
                        ->get();
...
// ci moderation wise mks & wt 
                $ciModWiseMksWtInfo = CiModerationMarking::join('ci_moderation_marking_lock', function($join) {
                        ...
                        ->whereIn('ci_moderation_marking.term_id', $termIdList)
...
// comdt moderation wise mks & wt 
                $comdtModWiseMksWtInfo = ComdtModerationMarking::join('comdt_moderation_marking_lock', function($join) {
                        ...
                        ->whereIn('comdt_moderation_marking.term_id', $termIdList)
                ...
                $muaPosnArr = $this->getMutualAssessmentPosition($request);
                ...
                if ($request->view == 'print') {
                    return view('report.individualProfile.print.profile')->with(compact('cmInfoData', 'cmArr', 'religionList', 'appointmentList', 'allAppointmentList', 'armsServiceList'
                    ...
                    $pdf = PDF::loadView('report.individualProfile.print.profile', compact('cmInfoData', 'cmArr', 'religionList', 'appointmentList', 'allAppointmentList', 'armsServiceList'
                                            , 'unitList', 'maritalStatusList', 'countriesVisitedList', 'othersInfoData', 'addressInfo'
                                            , 'divisionList', 'districtList', 'thanaList', 'civilEducationInfoData', 'serviceRecordInfoData'
                                            , 'defenceRelativeInfoData', 'courseList', 'organizationList', 'passportInfoData'
                                            , 'presentAddressInfo', 'presentDistrictList', 'presentThanaList', 'milCourseList'
                ...
                return view('report.individualProfile.profile')->with(compact('cmInfoData', 'cmArr', 'religionList', 'appointmentList', 'allAppointmentList', 'armsServiceList'
                                        , 'unitList', 'maritalStatusList', 'countriesVisitedList', 'othersInfoData', 'addressInfo'
                                        , 'divisionList', 'districtList', 'thanaList', 'civilEducationInfoData', 'serviceRecordInfoData'
                                        , 'defenceRelativeInfoData', 'courseList', 'organizationList', 'passportInfoData'
                                        , 'presentAddressInfo', 'presentDistrictList', 'presentThanaList', 'milCourseList'
...
//                }
                ...
                if ($request->view == 'print') {
                    return view('report.individualProfile.print.index')->with(compact('request', 'activeTrainingYearList', 'courseList', 'termList'
                                            , 'targetArr', 'qpArr', 'cmList'));
                } elseif ($request->view == 'pdf') {
                    $pdf = PDF::loadView('report.individualProfile.print.index', compact('request', 'activeTrainingYearList', 'courseList', 'termList'
                                            , 'targetArr', 'qpArr', 'cmList'))
                            ->setPaper('a4', 'landscape')
                            ->setOptions(['defaultFont' => 'sans-serif']);
                    ...
                    return Excel::download(new ExcelExport('report.individualProfile.print.index', compact('request', 'activeTrainingYearList', 'courseList', 'termList'
                                            , 'targetArr', 'qpArr', 'cmList')), $fileName . '.xlsx');
                }

                return view('report.individualProfile.index', compact('request', 'activeTrainingYearList', 'courseList', 'termList'
                                , 'targetArr', 'qpArr', 'cmList', 'spouseProfession'));
            }
        }

        return view('report.individualProfile.index')->with(compact('request', 'activeTrainingYearList', 'courseList', 'qpArr', 'cmList', 'spouseProfession')
        );
    }
    ...
    public function getCourse(Request $request) {
        ...
        $courseList = ['0' => __('label.SELECT_COURSE_OPT')] + $courseList;
        $html = view('report.individualProfile.getCourse', compact('courseList'))->render();
    ...
    public function getCm(Request $request) {
        ...
        $html = view('report.individualProfile.getCm', compact('cmList'))->render();
        return Response::json(['html' => $html]);
    }
    ...
    public function filter(Request $request) {
        ...
        if ($validator->fails()) {
            return redirect('individualProfileReport?generate=false&' . $url)
                            ->withInput()
                            ->withErrors($validator);
        }
        return redirect('individualProfileReport?generate=true&' . $url);
    }

    public function profile(Request $request, $id) {
        $url = 'generate=' . $request->generate . '&training_year_id=' . $request->training_year_id . '&course_id=' . $request->course_id . '&cm_id=' . $id;
        return redirect('individualProfileReport?' . $url);
    }
